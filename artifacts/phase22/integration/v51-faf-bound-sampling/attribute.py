...
identities = {}
for path in sorted(OUT.glob('SENT-*-worker-identity.json')):
    identity = read(path)
    assert identity['scanner'] == p['scanner'] and identity['verified_before_timer_and_target_snapshot']
    assert identity['imports'] == {n: str(Path(p['frozen_checkout'])/('src/sentinel/static/'+n+'.py')) for n in ['workers','typescript_path_flow']}
    identities[path.name.removesuffix('-worker-identity.json')] = {'sha256': sha(path), **identity}
assert set(identities) == {'SENT-012','SENT-014','SENT-015','SENT-016'}
