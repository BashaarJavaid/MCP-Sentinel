"""Proposed111-observation local exposed regression and compatibility sequence; no execution without a new approval."""
...
def verify():
    ...
    assert proposal["bounds"]["native_observations"] == len(proposal["order"]) == 111
    assert len({(x["group"], x["input_id"]) for x in proposal["order"]}) == 63
...
def run(out):
    ...
    try:
        ...
        assert len(packet["attempts"]) == proposal["bounds"]["native_observations"] == 111 and not packet["unstarted"]
        packet["passed"] = True
