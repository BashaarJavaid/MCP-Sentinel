...
EXPECTED='884d3763d8d63786e1726932951fd859cb6071c2';TITLE='Phase 22: Python correction verified; regression approval pending'
...
assert b['baseline_delivery']==EXPECTED and not a['technical_acceptance_received'] and not a['current_regression_approved'] and a['current_source_corpus_observations']==0
assert not (OUT/'execution-consumed.json').exists() and not (OUT/'evaluation-authorization.json').exists() and not (BASE/'v87-regression-results').exists()
...
for name,digest in read(BASE/'v85-class-call-boundary/local-checks.json')['candidate_engineering_files_sha256'].items():assert sha(ROOT/name)==digest,name
...
seal=read(BASE/'evidence-v103.json');assert sha(BASE/'evidence-v103.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: class-call boundary corrected; engineering in progress'
assert current['body']==(BASE/'v85-class-call-boundary/candidate-pr-body.md').read_text()
...
extra=[BASE/'evidence-v103.json',BASE/'evidence-v103.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v86-documentation-binding.*')]
...
print(run(['git','commit','-m','Deliver verified Python correction and exact exposed regression proposal [skip ci]']),flush=True)
...
receipt={'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'head':head,'base':'phase22/description-poisoning','draft':True,'open':True,'title':TITLE,'body_sha256':sha(OUT/'pr-body-delivery.md'),'evaluation_proposal_sha256':b['evaluation_proposal_sha256'],'scanner':b['corrected_scanner'],'seal103_sha256':seal['sha256'],'readback_history_sha256':sha(OUT/'delivery-readback-history.json'),'documentation_binding_sha256':sha(OUT/'documentation-binding.json'),'protected_user_files_main_worktrees_preserved':True,'current_source_observations':0,'proposed_observations':111,'proposed_input_records':63,'proposed_ordered_pairs':48,'new_numerical_approval_received':False,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False,'effective_scope_disposition':{'id':'V86-REVIEW-DELIVERY','disposition':'passed','basis':'Affected checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body verified. This resolves proposal delivery only; exact regression approval/results, updated review, explicit human acceptance and actual accepted closeout remain.'},'effective_additional_scope_dispositions':counts,'provenance':'Supplemental actual readback outside its own seal/commit; preserve in next authorized evidence seal without infinite self-receipt commits.'}
