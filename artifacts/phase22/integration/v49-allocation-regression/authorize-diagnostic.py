...
assert sha(ASSETS/'diagnostic-proposal.json') == 'e902b48857e0cfca4aaffe3fe551511952c064f57c902fe3850d6892b05ec9c4'
assert not (ASSETS/'diagnostic-authorization.json').exists()
assert not (ASSETS/'diagnostic-consumed.json').exists()
assert not (ASSETS.parent/'v50-faf-allocation-sampling').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '2933d39493b8cf7d52bfb8e1ffdca879d20d5c1d'
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v50-', 'v49-', 'v48-', 'v47-', 'v46-', 'v45-', 'v44-', 'v43-', 'phase22-check.py', 'mcp-phase22-four-fresh-']
...
receipt = {'approved': True, 'user_decision': 'approved', 'recorded_at': datetime.now(timezone.utc).isoformat(),
    'decision_context': 'User approved the exact single CPU-sampled FAF rules-only diagnostic delivered at2933d39, with300-second whole-input maximum,15-second cleanup,zero optimization/retries/comparators/paid calls/target execution. The closed205-observation budget remains closed.',
