...
def main():
    assert json.loads((BASE / "v67-invalidation-regression/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v67-invalidation-regression/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v67-invalidation-regression/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v68-invalidation-sampling").exists()
    archive, manifest = BASE / "evidence-v95.tar.gz", BASE / "evidence-v95.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v94.json",
        ...
        "status": "Approved regression at4145d35 timed out on its first FAF input after1800.008716seconds; one incomplete,zero reports,204unstarted closed,zero remaining. Cleanup verified. All383requirements and previous failures retained. One parent-and-worker profile is prepared,unapproved and unexecuted. Engineering remains source-bound at4145d35. Phase22 incomplete; zero new profiles or paid calls.",
        "verification": (
            "All 94 prior archives and every new member verified afte"
