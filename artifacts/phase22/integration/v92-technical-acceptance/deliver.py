...
EXPECTED='f2d1b0f34d372f0331b06fd2319514d3b45ab811';TITLE='Phase 22: Proxmox limitation accepted; technical acceptance pending'
...
assert read(BASE/'v90-regression-results/completion-verification.json')['budget_closed'] and read(OUT/'proxmox-limitation-acceptance.json')['approved'] and a['acceptance_packet_ready']
...
seal=read(BASE/'evidence-v106.json');assert sha(BASE/'evidence-v106.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: Proxmox remains unsupported; failure disposition pending'
assert current['body']==(BASE/'v91-proxmox-disposition/pr-body-delivery.md').read_text()
...
extra=[BASE/'evidence-v106.json',BASE/'evidence-v106.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v92-documentation-binding.*')]
...
print(run(['git','commit','-m','Deliver complete Phase22 technical acceptance review and Proxmox disposition [skip ci]']),flush=True)
...
receipt={'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'head':head,'base':'phase22/description-poisoning','draft':True,'open':True,'title':TITLE,'body_sha256':sha(OUT/'pr-body-delivery.md'),'acceptance_proposal_sha256':b['acceptance_proposal_sha256'],'scanner':b['corrected_scanner'],'seal106_sha256':seal['sha256'],'readback_history_sha256':sha(OUT/'delivery-readback-history.json'),'documentation_binding_sha256':sha(OUT/'documentation-binding.json'),'protected_user_files_main_worktrees_preserved':True,'current_source_observations':6,'entire_ordered_pairs_equal':3,'proposed_observations':0,'failed_gate_disposition_received':True,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False,'effective_scope_disposition':{'id':'V92-REVIEW-DELIVERY','disposition':'passed','basis':'Affected checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body verified. This resolves complete technical acceptance review delivery. Proxmox limitation/deferral is already explicitly accepted. Separate human Phase22technical acceptance and subsequent verified accepted closeout remain.'},'effective_additional_scope_dispositions':counts,'provenance':'Supplemental actual readback outside its own seal/commit; preserve in next authorized evidence seal without infinite self-receipt commits.'}
