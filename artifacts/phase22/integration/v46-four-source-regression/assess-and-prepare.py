...
BASE = OUT.parent
ROOT = OUT.parents[3]
PREVIOUS = BASE/'v45-four-source-recovery'
...
assessment = {'recorded_at': datetime.now(timezone.utc).isoformat(), 'scanner': p['scanner'],
    ...
    'source_explanation': 'The whole-input supervisor reached300seconds and sent SIGTERM. Its handler raises SystemExit; measure initializes an incomplete entry with reasonNone and writes results in finally. SystemExit bypasses exceptException, explaining the preserved incomplete/null reason and absence of report files. This is not a completed native exit3 report or zero-finding result. No retained stack or stage trace establishes the expensive operation.',
...
files = [PREVIOUS/'evaluation-proposal.json', PREVIOUS/'evaluation-authorization.json', PREVIOUS/'execution-consumed.json', PREVIOUS/'evaluate.py',
         ...
         BASE/'v43-four-fresh-preparation/runner.py', BASE/'v20-linux-diagnostic-v1/runner.py']
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': p['scanner'], 'lock_sha256': p['lock_sha256'], 'frozen_checkout': p['frozen_checkout'],
    ...
    'purpose': 'Locate expensive scanner operations on the sole timed-out FAF vulnerable input before considering a code change. Existing output contains no stage/stack evidence, so source inspection alone cannot establish a performance root cause.',
    'command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'diagnostic.py')],
    'initial_cwd': str(ROOT), 'output': str(BASE/'v47-faf-sampling'),
    ...
    'environment': 'Existing locked local macOS environment; frozen17b4784 source; no dependency/resource/runner upgrade and no overlapping owned heavy work.',
