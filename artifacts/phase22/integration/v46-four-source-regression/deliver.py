...
EXPECTED = '0135461d26688fc286e97a486cdef9336bac6eb1'
TITLE = 'Phase 22: FAF regression timed out; diagnostic approval pending'
...
assert sha(BASE/'evidence-v74.json') == binding['evidence_seal']['index_sha256']
...
with tarfile.open(BASE/seal['archive']) as archive:
    assert len(archive.getmembers()) == len(seal['files']) == 112
...
for name, digest in read(OLD/'local-checks.json')['candidate_engineering_files_sha256'].items():
    assert sha(ROOT/name) == digest, name
current = pr()
guard(current)
assert current['headRefOid'] == EXPECTED and current['body'] == (OLD/'pr-body-reviewed.md').read_text()
...
extra = [BASE/'evidence-v74.json', BASE/'evidence-v74.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'diagnostic.py', OUT/'sampling-worker.py', OUT/'sampler-selfcheck.py', OUT/'validate.py', OUT/'assess-and-prepare.py']
extra.extend(BASE.glob('v46-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain FAF timeout and bounded diagnostic proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal74_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'corpus_budget': binding['corpus_budget'], 'diagnostic_executions': 0, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V46-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 133, 'historical_closure_proposals': 6, 'unresolved': 5},
    'provenance': 'Supplemental post-commit readback, outside seal74 and the commit it verifies; retain in next authorized seal.'}
