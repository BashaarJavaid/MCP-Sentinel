...
assert sha(ASSETS/'diagnostic-proposal.json') == '3cb2a24fdd718c4cee63c84a9d10c3f8694dc004d9ef621651e9ced93f7ba575'
assert not (ASSETS/'diagnostic-authorization.json').exists()
assert not (ASSETS/'diagnostic-consumed.json').exists()
assert not (ASSETS.parent/'v47-faf-sampling').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '73d7b61129e2e8ee8c49e491e0a436390ae2142c'
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v47-', 'v46-', 'v45-', 'v44-', 'v43-', 'phase22-check.py', 'mcp-phase22-four-fresh-']
...
receipt = {'approved': True, 'user_decision': 'approved', 'recorded_at': datetime.now(timezone.utc).isoformat(),
    'decision_context': 'User approved the exact single CPU-sampled FAF rules-only diagnostic delivered at73d7b61, with300-second whole-input maximum,15-second cleanup,zero optimization/retries/comparators/paid calls/target execution. The closed205-observation budget remains closed.',
