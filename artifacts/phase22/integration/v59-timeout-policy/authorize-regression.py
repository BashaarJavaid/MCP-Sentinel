...
assert sha(ASSETS/'evaluation-proposal.json') == '0a07544a9db0396780d686af952ab2057cc78f8df2f6897128c714ac4126d164'
assert sha(ASSETS/'launcher-binding.json') == '4285befc69588b61361dd4c983103a0e1c7e5db2c62921961d9e796d3c8b3420'
assert not (ASSETS/'evaluation-authorization.json').exists()
assert not (ASSETS/'execution-consumed.json').exists()
assert not (ASSETS.parent/'v60-long-timeout-regression').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '24ff6a0f070ec26757383d7bc84930fcc4a6684f'
assert not subprocess.check_output(['git', 'diff', 'HEAD'], cwd=ROOT)
delivery = read(ASSETS/'delivery-verification.json')
assert delivery['passed'] and delivery['head'] == '24ff6a0f070ec26757383d7bc84930fcc4a6684f' and delivery['evaluation_proposal_sha256'] == sha(ASSETS/'evaluation-proposal.json')
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v60-', 'v59-', 'v58-', 'v57-', 'v56-', 'v55-', 'v54-', 'v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v45-', 'v46-', 'v43-', 'v44-', 'phase22-check.py', 'mcp-phase22-four-fresh-', 'v42-', 'v41-', 'v40-fresh-v7/', 'v38-guard-regression/']
