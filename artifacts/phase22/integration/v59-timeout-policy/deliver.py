"""Deliver verified timeout policy and the unapproved longer-timeout proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = '7bf4c6e1cf0c8d83229b1273f80737e7cacc5b82'
TITLE = 'Phase 22: 30-minute deadline verified; evaluation approval pending'
...
assert sha(BASE/'evidence-v87.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v87.json', BASE/'evidence-v87.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v59-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Verify 30-minute static policy and prepare bounded exposed evaluation [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal87_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_corpus_observations': 0, 'corpus_budget': binding['corpus_budget'], 'profiles': 0, 'evaluation_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V59-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 214, 'historical_closure_proposals': 6, 'unresolved': 14},
    'provenance': 'Supplemental post-commit readback, outside seal87 and the commit it verifies; retain in next authorized seal.'}
