...
source = b / "v59-candidate-quality"
...
p = {
    ...
    "classification": "Fresh current-source engineering for the approved shared-timeout policy candidate 7bf4c6e. All four optional corpus jobs skipped. Retained historical reproductions use8824014; no fresh or exposed corrected corpus observation is inferred.",
}
local_coverage = json.loads((b / "v59-full-suite-coverage.json").read_text())["totals"]
...
assert len(p["quality"]) == 12 and all(
    r["passed"] == 2403 and r["skipped"] == 36 for r in p["quality"]
)
tracked = set(
...
checkout = json.loads((b / "v59-timeout-policy/checkout-binding.json").read_text())
...
p["checkout_binding_sha256"] = sha((b / "v59-timeout-policy/checkout-binding.json").read_bytes())
p["actual_synthetic_merge"] = checkout["synthetic_merge"]
p["complete_checkout_tree_equals_head"] = True
out = b / "v59-candidate-quality-audit"
