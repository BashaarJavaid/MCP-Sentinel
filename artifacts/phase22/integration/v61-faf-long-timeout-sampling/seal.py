...
def main():
    assert json.loads((BASE / "v61-faf-long-timeout-sampling/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v61-faf-long-timeout-sampling/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v61-faf-long-timeout-sampling/budget-closed.json").read_text())["budget_closed"]
    assert not (BASE / "v62-shared-tool-discovery").exists()
    archive, manifest = BASE / "evidence-v89.tar.gz", BASE / "evidence-v89.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v88.json",
        ...
        "status": "Approved one-use 1800-second FAF profile at 7bf4c6e timed out with four verified worker identities and valid partial samples; no report, cleanup passed, zero remaining. All 335 requirements and earlier failures retained. One source-only shared tool-discovery reuse attempt is prepared, unapproved and unstarted. Product/tests retain actual 2403/36 engineering. Phase 22 remains incomplete; no human technical acceptance.",
        "verification": (
            "All 88 prior archives and every new member verified afte"
