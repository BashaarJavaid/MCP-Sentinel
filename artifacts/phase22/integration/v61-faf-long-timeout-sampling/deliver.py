"""Deliver the assessed profile and unapproved source-only reuse proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = '777c7526f18733b7ca4b49e3caf690f02d25adf6'
TITLE = 'Phase 22: profile assessed; shared discovery proposal pending'
...
assert sha(BASE/'evidence-v89.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v89.json', BASE/'evidence-v89.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v61-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Assess 30-minute profile and prepare shared tool-discovery reuse [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal89_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_corpus_observations': 1, 'corpus_budget': binding['corpus_budget'], 'profiles': 1, 'optimization_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V61-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 225, 'historical_closure_proposals': 6, 'unresolved': 15},
    'provenance': 'Supplemental post-commit readback, outside seal89 and the commit it verifies; retain in next authorized seal.'}
