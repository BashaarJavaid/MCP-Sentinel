...
def main():
    assert json.loads((BASE / "v93-accepted-closeout/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v93-accepted-closeout/owned-final.json").read_text())["owned_processes"] == []
    ...
    assert json.loads((BASE / "v93-accepted-closeout/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v107.tar.gz", BASE / "evidence-v107.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v106.json",
        ...
        "status": "Explicit human Phase22technical acceptance recorded against delivered V92proposal f2030fbd.../792d922/seal106. Accepted493-row closeout audit preserves30accepted limitations,2paid/pilotdeferrals and all actual failed measurements. Unchanged5142a3fengineering2596tests/36skips/12hostedsuites/29jobs/docs,sixzero-call requests,19runtimebindings. No new observations or paidcalls. Proxmoxunsupported/recoverydeferred,ordinaryFAF1800completionunestablished,V68unapproved/unimplemented. Actual final delivery alone remains; supplemental verification resolves9delivery-dependent rows without backdating.",
        "verification": (
            "All 106 prior archives and every new member verified afte"
