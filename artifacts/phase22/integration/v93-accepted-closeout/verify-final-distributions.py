...
assert not subprocess.check_output(['git','diff','5142a3f','--','src','tests','scripts','schemas','pyproject.toml','uv.lock','.github','action.yml','Makefile','LICENSE','CHANGELOG.md'])
tracked=set(subprocess.check_output(['git','ls-tree','-r','--name-only','HEAD'],text=True).splitlines())
hosted=list((BASE/'v88-candidate-quality/artifacts/ci/portunusmcp-sentinel-dist').glob('*.whl'));assert len(hosted)==1
...
for path in sorted((OUT/'final-distributions').iterdir()):
 ...
 for n,source in mapping.items():
  ...
  if source!='README.md':assert subprocess.check_output(['git','show','5142a3f:'+source])==members[n]
  checked.append({'member':n,'source_path':source,'sha256':sha(members[n]),'binding':'current status-only documentation' if source=='README.md' else 'exact hosted5142a3f Git blob'})
...
assert len(rows)==2
p={'hosted_source':'5142a3fbd5bcff63719df6cce3a30543214e7d27','scanner':'5142a3fbd5bcff63719df6cce3a30543214e7d27','README_sha256':sha((ROOT/'README.md').read_bytes()),'distributions':rows,'wheel_product_schema_fixture_members_byte_identical_to_hosted':True,'qualification':'Final README status documentation changes package long-description metadata only. Newly built wheel/sdist source members match current files; every non-README source member equals its tested5142a3f Git blob. Hosted installed-wheel matrix, Docker and isolation results retain actual5142a3f artifacts; no new hosted run is claimed for docs-only delivery.'}
