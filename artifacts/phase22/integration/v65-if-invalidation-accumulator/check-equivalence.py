...
passed=all(r['equivalent'] for r in controls)
packet={'passed':passed,'ordinary_comparisons':len(ordinary),'ordinary':ordinary,'controls':controls,'avoided_work_verified_for_ordinary_cases':True,'candidate_sha256':hashlib.sha256((OUT/'candidate-typescript-path-flow.py').read_bytes()).hexdigest(),'baseline_sha256':hashlib.sha256(BASE.read_bytes()).hexdigest(),'corpus_observations':0,'profiles':0,'paid_calls':0,'qualification':'Entire flow/environment/Value contents and named relevant set aliases compared. Instrumentation fields are included, not excluded. A failed adversarial prerequisite closes the single approved attempt regardless of ordinary-case passes; no real-target reachability is inferred.'}
with (OUT/'equivalence.json').open('x') as f:json.dump(packet,f,indent=2);f.write('\n')
print(json.dumps({'passed':passed,'ordinary_comparisons':len(ordinary),'controls':[{k:r[k] for k in ['control','equivalent']} for r in controls],'representative_operations':representative},indent=2))
