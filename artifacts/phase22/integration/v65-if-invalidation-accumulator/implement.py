...
record={'recorded_at':datetime.now(timezone.utc).isoformat(),'approval_sha256':sha(OUT/'authorization.json'),'proposal_sha256':a['proposal_sha256'],'attempts_consumed':1,'remaining_attempts':0,'bounds':a['bounds'],'source_uses':uses,'source_assessment':'Current production writers initialize sets, add/update invalidations and restore/union per-arm baselines. No production removal or raw branch-set export is found in these attribute occurrences. This source observation is not a waiver of the separately required adversarial alias/shrinking controls.','paid_calls':0}
