...
head = '0f36867a037dbb13dc19e504b9ebceab2b75b407'
assert git('rev-parse', 'HEAD') == head
assert git('branch', '--show-current') == 'phase22/integration'
assert not git('diff', 'HEAD') and not git('diff', '--cached')
proposal_path = BASE / 'v64-shared-discovery-sampling/optimization-proposal.json'
proposal = json.loads(proposal_path.read_text())
assert sha(proposal_path) == 'd8b425ba9bc7546c1f8dfa0cee9172447617b28e02da25ec89af694654f5077e'
intake = json.loads((BASE / 'v64-shared-discovery-sampling/documentation-binding.json').read_text())
...
assert remote['title']=='Phase 22: profile assessed; invalidation proposal pending' and remote['body']==(BASE/'v64-shared-discovery-sampling/pr-body-delivery.md').read_text()
...
record = {
    ...
    'decision': 'approved',
    'decision_context': 'The user approved the immediately preceding exact one-attempt If invalidation-accumulator proposal, with full synthetic/engineering verification and zero corpus/profile/paid calls; the 1800-second deadline is unchanged.',
