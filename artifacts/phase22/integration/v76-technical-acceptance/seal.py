...
def main():
    assert json.loads((BASE / "v76-technical-acceptance/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v76-technical-acceptance/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v76-technical-acceptance/audit.json").read_text())["regression_approved"]
    assert not json.loads((BASE / "v76-technical-acceptance/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v99.tar.gz", BASE / "evidence-v99.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v98.json",
        ...
        "status": "Current cec0322 narrow gates pass: V73 four24/24 and V75 prior-language181/181, all95 entire ordered pairs; both approved budgets closed. Ordinary FAF1800-second completion remains unestablished; all6 uncapped FAF observations exceeded that policy. All422 requirements, original fresh/native/profile/equivalence failures and proposed historical limitations remain. Unchanged2450/36 engineering,6zero-callproduction requests and19runtimebindings; affected docs/packages pass. Explicit human technical acceptance and verified accepted closeout pending. V68optimization unapproved/unimplemented; Git/paid/pilot deferrals unchanged; zero new paid calls.",
        "verification": (
            "All 98 prior archives and every new member verified afte"
