...
root = Path.cwd()
out = root / "artifacts/phase22/integration/v33-fresh-v6"
...
for path, expected in [
    ...
    if path.endswith("frozen-f85a90f"):
        staged=binding["authorized_staged_frozen_artifacts_sha256"]
        assert set(subprocess.check_output(["git","ls-files","--others","--exclude-standard"],cwd=path,text=True).splitlines())==set(staged)
        for name,digest in staged.items():assert hashlib.sha256((Path(path)/name).read_bytes()).hexdigest()==digest
    else:
        assert not subprocess.check_output(["git", "status", "--porcelain"], cwd=path)
assert (out.parent / "v32-fresh-v6/evaluation-authorization.json").exists()
assert (out.parent / "v32-fresh-v6/execution-consumed.json").exists()
...
receipt = {
    ...
    "new_paid_calls": 0,
    "current_source_corpus_observations": 79,
    "native_observations": 74,
    "semgrep_observations": 5,
    "prior_scanner_regression_observations_this_continuation": 0,
    "new_evaluation_approved": True,
    "new_evaluation_executed": True,
    "new_corpus_observations": 15,
    "fresh_evaluation_and_acceptance_proposal_delivered": True,
    "fresh_detection_prerequisite_met": True,
    "fixed_guard_recognition_established": False,
