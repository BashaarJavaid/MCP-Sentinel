...
assert not subprocess.check_output(['git','diff','1948bf9','--','src','tests','scripts','schemas','pyproject.toml','uv.lock','.github','action.yml','Makefile','LICENSE','CHANGELOG.md'])
tracked=set(subprocess.check_output(['git','ls-tree','-r','--name-only','HEAD'],text=True).splitlines())
hosted=list((BASE/'v62-candidate-quality/artifacts/ci/portunusmcp-sentinel-dist').glob('*.whl'));assert len(hosted)==1
...
for path in sorted((OUT/'final-distributions').iterdir()):
 ...
 for n,source in mapping.items():
  ...
  if source!='README.md':assert subprocess.check_output(['git','show','1948bf9:'+source])==members[n]
  checked.append({'member':n,'source_path':source,'sha256':sha(members[n]),'binding':'current status-only documentation' if source=='README.md' else 'exact hosted1948bf9 Git blob'})
...
assert len(rows)==2
p={'hosted_source':'1948bf942babce155b701f5b22ed941eb16d99d0','scanner':'1948bf942babce155b701f5b22ed941eb16d99d0','README_sha256':sha((ROOT/'README.md').read_bytes()),'distributions':rows,'wheel_product_schema_fixture_members_byte_identical_to_hosted':True,'qualification':'Final README status documentation changes package long-description metadata only. Newly built wheel/sdist source members match current files; every non-README source member equals its tested1948bf9 Git blob. Hosted installed-wheel matrix, Docker and isolation results retain actual1948bf9 artifacts; no new hosted run is claimed for docs-only delivery.'}
