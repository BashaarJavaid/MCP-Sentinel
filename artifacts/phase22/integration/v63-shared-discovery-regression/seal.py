...
def main():
    assert json.loads((BASE / "v63-shared-discovery-regression/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v63-shared-discovery-regression/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v63-shared-discovery-regression/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v64-shared-discovery-sampling").exists()
    archive, manifest = BASE / "evidence-v91.tar.gz", BASE / "evidence-v91.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v90.json",
        ...
        "status": "Approved205-observation regression at1948bf9 stopped on first FAF input at1800.010893seconds; no report,204unstarted closed,cleanup passed. All352requirements and earlier failures retained. One parent-and-worker profile is prepared,unapproved and unexecuted. Product retains2418tests/36skips engineering; Phase22 incomplete with no human technical acceptance.",
        "verification": (
            "All 90 prior archives and every new member verified afte"
