...
BASE = OUT.parent
OLD = OUT
EXPECTED = 'c7b20a30f2a061c00eb8a6ffcc1413a211c01fff'
TITLE = 'Phase 22: shared discovery regression timed out; diagnostic pending'
...
assert not (BASE/'v64-shared-discovery-sampling').exists()
seal = read(BASE/'evidence-v91.json')
assert sha(BASE/'evidence-v91.json') == binding['evidence_seal']['index_sha256']
...
current = pr()
guard(current)
assert current['headRefOid'] == EXPECTED and current['body'] == (BASE/'v62-shared-tool-discovery/pr-body-delivery.md').read_text()
...
extra = [BASE/'evidence-v91.json', BASE/'evidence-v91.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v63-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Assess shared discovery timeout and prepare bounded diagnostic [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal91_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    ...
    'effective_scope_disposition': {'id': 'V63-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 241, 'historical_closure_proposals': 6, 'unresolved': 16},
    'provenance': 'Supplemental post-commit readback, outside seal91 and the commit it verifies; retain in next authorized seal.'}
