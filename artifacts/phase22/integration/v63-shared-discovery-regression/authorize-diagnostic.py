...
assert sha(ASSETS/'diagnostic-proposal.json') == '6e8e955b82f1c17ea71110c1b348fa46dc546c127fa1588578a9a7bca932652b'
assert not (ASSETS/'diagnostic-authorization.json').exists()
assert not (ASSETS/'diagnostic-consumed.json').exists()
assert not (ASSETS.parent/'v64-shared-discovery-sampling').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '7d6e01a0e8c9e4e5f81a8fd195599dac6b7aa8ab'
...
assert remote['headRefOid'] == '7d6e01a0e8c9e4e5f81a8fd195599dac6b7aa8ab'
assert remote['title'] == 'Phase 22: shared discovery regression timed out; diagnostic pending' and remote['body'] == (ASSETS/'pr-body-delivery.md').read_text()
...
for name,digest in read(ASSETS.parent/'v62-shared-tool-discovery/local-checks.json')['candidate_engineering_files_sha256'].items():
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v64-', 'v63-', 'v62-', 'v61-', 'v60-', 'v59-', 'v58-', 'v57-', 'v56-', 'v55-', 'v54-', 'v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v46-', 'v45-', 'v44-', 'v43-', 'phase22-check.py', 'mcp-phase22-four-fresh-']
