...
root=Path('/private/tmp/mcp-phase22-options');out=root/'artifacts/phase22/integration/v63-shared-discovery-regression/sampler-selfcheck';out.mkdir(exist_ok=False);worker=out/'worker.py';shutil.copyfile(str(root/'artifacts/phase22/integration/v63-shared-discovery-regression/corrected-sampling-worker.py'),worker);sys.argv=['synthetic','synthetic'];n=runpy.run_path(str(worker),run_name='selfcheck')
identity=n['verify_scanner']();assert identity['scanner']['revision']=='1948bf942babce155b701f5b22ed941eb16d99d0'
...
import subprocess
bad=out/'wrong-root-worker.py';bad.write_text(worker.read_text().replace('/private/tmp/mcp-phase22-frozen-1948bf9','/private/tmp/mcp-phase22-frozen-7bf4c6e'))
