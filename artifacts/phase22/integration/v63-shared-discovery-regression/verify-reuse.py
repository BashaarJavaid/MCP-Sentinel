...
assert local['states'] == {'passed': 2418, 'skipped': 36}
...
assert not subprocess.check_output(['git', 'diff', '1948bf9', '--', 'src', 'tests', 'scripts', 'schemas', '.github', 'uv.lock', 'pyproject.toml', 'CHANGELOG.md'])
quality = read(OUT.parent/'v62-candidate-quality-audit/packet.json')
assert quality['source'] == p['scanner']['revision'] and len(quality['quality']) == 12
assert all(r['passed'] == 2418 and r['skipped'] == 36 for r in quality['quality'])
replay = read(OUT.parent/'v62-production-capture-revalidation/packet.json')
