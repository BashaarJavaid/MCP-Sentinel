...
p=BASE/'README.md';text=p.read_text();assert text.count('batches 1–100')==1;text=text.replace('batches 1–100','batches 1–101',1).replace('## Current unseen-preparation evidence seal 100','## Historical unseen-preparation evidence seal 100',1)
...
review=['audit.json','summary.md','evaluation-proposal.json','scoring-rubric.json','prior-row-preservation.json','final-checks.json','final-distributions.json','command-provenance.json','check-failure-assessment.json','owned-final.json','pr-body-delivery.md','seal-execution.json','identity-correction-verification.json','collection-checks.json','launcher-binding.json']
paths=[OUT/name for name in review]+[BASE/'v78-unseen-results/stop-assessment.json',BASE/'v77-unseen-preparation/selection-record.json']+[p for p in (ROOT/'artifacts/phase22/corpus-unseen-v1').rglob('*') if p.is_file()]
