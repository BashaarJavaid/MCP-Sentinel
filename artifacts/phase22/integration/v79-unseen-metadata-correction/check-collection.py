...
order=[{'group':'synthetic','input_id':'fixture-'+str(n),'batch':batch} for batch in ['first','repeat'] for n in range(6)]
proposal={'order':order,'scanner':{'synthetic':True},'bounds':{'internal_stop_minutes':378,'native_observations':12}}
results=[]
...
for scenario in scenarios:
 with tempfile.TemporaryDirectory(prefix='phase22-unseen-loop-synthetic-') as tmp:
  ...
  p={**proposal,'bounds':{**proposal['bounds'],'internal_stop_minutes':0 if scenario=='insufficient_outer_allowance' else 378}}
  def supervise(command,log,limit):
   ...
   if scenario=='ordered_repeat_failure' and calls[0]==7:report['warnings'].append('changed')
  ...
  with patch.object(r,'ASSETS',assets),patch.object(r,'PROPOSAL',assets/'proposal.json'),patch.object(r,'APPROVAL',assets/'approval.json'),patch.object(r,'approved',return_value=p),patch.object(r.supervisor,'supervise',supervise),patch.object(r.supervisor,'selfcheck',lambda _:None),patch.object(r,'selfcheck',lambda:None),(assets/'stdout.log').open('w') as log,redirect_stdout(log):
   try:r.run(raw)
  ...
  packet=json.loads((raw/'packet.json').read_text());expected=12 if scenario=='complete_with_failed_conditions' else 7 if scenario=='ordered_repeat_failure' else 0 if scenario=='insufficient_outer_allowance' else 2
  assert len(packet['attempts'])==expected and len(packet['unstarted'])==12-expected
