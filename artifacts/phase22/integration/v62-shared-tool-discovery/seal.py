...
def main():
    assert json.loads((BASE / "v62-shared-tool-discovery/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v62-shared-tool-discovery/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v62-shared-tool-discovery/audit.json").read_text())["optimization_budget_closed"]
    assert not (BASE / "v63-shared-discovery-regression").exists()
    archive, manifest = BASE / "evidence-v90.tar.gz", BASE / "evidence-v90.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v89.json",
        ...
        "status": "One approved source-only completed discovery reuse at1948bf9 passes synthetic and full local/hosted engineering,2418tests/36skips. All345 requirements and earlier failures retained. Exact205-observation regression is prepared,unapproved and unexecuted. Phase22 remains incomplete; no human technical acceptance.",
        "verification": (
            "All 89 prior archives and every new member verified afte"
