...
source = b / "v62-candidate-quality"
...
p = {
    ...
    "classification": "Fresh current-source engineering for the approved shared tool-discovery reuse candidate 1948bf9. All four optional corpus jobs skipped. Retained historical reproductions use8824014; no fresh or exposed corrected corpus observation is inferred.",
}
local_coverage = json.loads((b / "v62-full-suite-coverage.json").read_text())["totals"]
...
assert len(p["quality"]) == 12 and all(
    r["passed"] == 2418 and r["skipped"] == 36 for r in p["quality"]
)
tracked = set(
...
checkout = json.loads((b / "v62-shared-tool-discovery/checkout-binding.json").read_text())
...
p["checkout_binding_sha256"] = sha((b / "v62-shared-tool-discovery/checkout-binding.json").read_bytes())
p["actual_synthetic_merge"] = checkout["synthetic_merge"]
p["complete_checkout_tree_equals_head"] = True
out = b / "v62-candidate-quality-audit"
