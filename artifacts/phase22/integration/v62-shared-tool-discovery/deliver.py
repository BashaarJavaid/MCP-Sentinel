"""Deliver verified source-only discovery reuse and unapproved regression proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = OUT
EXPECTED = '1948bf942babce155b701f5b22ed941eb16d99d0'
TITLE = 'Phase 22: shared discovery verified; regression approval pending'
...
assert not (BASE/'v63-shared-discovery-regression').exists()
seal = read(BASE/'evidence-v90.json')
assert sha(BASE/'evidence-v90.json') == binding['evidence_seal']['index_sha256']
...
for name, digest in read(OUT/'local-checks.json')['candidate_engineering_files_sha256'].items():
...
extra = [BASE/'evidence-v90.json', BASE/'evidence-v90.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v62-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Verify shared discovery and prepare bounded regression [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal90_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 1, 'current_source_corpus_observations': 0, 'corpus_budget': binding['corpus_budget'], 'profiles': 0, 'optimization_approved': True, 'regression_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V62-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 235, 'historical_closure_proposals': 6, 'unresolved': 15},
    'provenance': 'Supplemental post-commit readback, outside seal90 and the commit it verifies; retain in next authorized seal.'}
