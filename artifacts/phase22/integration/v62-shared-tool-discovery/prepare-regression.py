...
quality = read(BASE/'v62-candidate-quality/packet.json')
...
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': freeze['scanner'],
    ...
    'purpose': 'Evaluate the completed-discovery reuse candidate and still-unvalidated four-source recovery with TypeScript/Python compatibility. The approved 1800-second shared policy is unchanged. All rule-specific analysis executes. Synthetic reuse proves no native speedup or completion. Preserve all earlier source-bound fresh, native, profile and singleton failures.',
    'runner': str((OUT/'evaluate.py').relative_to(ROOT)),
    'prepared_command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'evaluate.py'), 'run', str(BASE/'v63-shared-discovery-regression/raw')],
