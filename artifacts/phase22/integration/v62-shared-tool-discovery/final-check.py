...
checks=['local-checks','hosted-audit','freeze','proposal-preparation','frozen-source-validation','launcher-binding','missing-approval','reconciliation','review-summary','final-docs','final-artifacts','final-build','final-distributions','owned-final','coverage-temp-retention']
for n in checks:assert read(base/('v62-'+n+'.json'))['exit_code']==0,n
...
old=read(base/'v61-faf-long-timeout-sampling/audit.json'); paths=set()
...
assert not (out/'evaluation-authorization.json').exists() and not (out/'execution-consumed.json').exists() and not (base/'v63-shared-discovery-regression').exists()
a=read(out/'audit.json');assert a['requirements']==old['requirements'] and len(a['additional_scope_requirements'])==256
for p,c in zip(old['additional_scope_requirements'],a['additional_scope_requirements']):assert p==c or c.get('previous_row')==p
assert len(a['current_requirement_interpretations'])==335
...
commands={}
for p in sorted(base.glob('v62-*.json')):
 row=read(p)
