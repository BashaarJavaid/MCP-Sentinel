...
source = b / "v72-candidate-quality"
...
p = {
    ...
    "classification": "Fresh current-source engineering for approved-contract constructor parameter-property and URL guard correctness at cec0322. All four optional corpus jobs skipped. Retained historical reproductions use 8824014; no fresh or exposed corrected corpus observation is inferred.",
}
local_coverage = json.loads((b / "v72-full-suite-coverage.json").read_text())["totals"]
p["coverage_metric_note"] = "Hosted Total coverage is the combined statement-and-branch metric used by configured coverage.py. The legacy field name branch_coverage_percent refers to that branch-enabled total, not branch-only coverage."
p["local_combined_coverage_percent"] = local_coverage["percent_covered"]
p["local_branch_only_coverage_percent"] = local_coverage["percent_branches_covered"]
...
assert len(p["quality"]) == 12 and all(
    r["passed"] == 2450 and r["skipped"] == 36 for r in p["quality"]
)
tracked = set(
...
checkout = json.loads((b / "v72-prototype-property-correction/checkout-binding.json").read_text())
...
p["checkout_binding_sha256"] = sha((b / "v72-prototype-property-correction/checkout-binding.json").read_bytes())
p["actual_synthetic_merge"] = checkout["synthetic_merge"]
p["complete_checkout_tree_equals_head"] = True
out = b / "v72-candidate-quality-audit"
