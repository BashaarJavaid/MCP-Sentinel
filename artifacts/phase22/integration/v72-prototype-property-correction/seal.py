...
def main():
    assert json.loads((BASE / "v72-prototype-property-correction/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v72-prototype-property-correction/owned-final.json").read_text())["owned_processes"] == []
    assert not json.loads((BASE / "v72-prototype-property-correction/audit.json").read_text())["regression_approved"]
    assert not (BASE / "v72-prototype-property-correction/evaluation-authorization.json").exists()
    assert not (BASE / "v69-ts-literal-key-reuse").exists()
    archive, manifest = BASE / "evidence-v98.tar.gz", BASE / "evidence-v98.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v97.json",
        ...
        "status": "Ordinary constructor parameter-property and derived URL guard corrections atcec0322 pass full engineering:2450tests/36skips locally/all12hosted suites,29normal jobs/docs. Initial prototype boundary failure and interrupted/cancelled42d6c7c engineering remain retained. New24-observation uncapped four-repository proposal is unapproved/unexecuted;181prior-language observations excluded. All407requirements, earlier failures and unaccepted closures remain. Normal1800-second policy unchanged;V68optimization unapproved;Phase22incomplete;zero new paid calls.",
        "verification": (
            "All 97 prior archives and every new member verified afte"
