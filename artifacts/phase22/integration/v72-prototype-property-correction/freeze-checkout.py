...
fixed=Path('/private/tmp/mcp-phase22-frozen-cec0322');revision='cec0322e904bbf63c33cd95796e7289101a93e5d';assert not fixed.exists()
...
assert json.loads((OUT.parent/'v72-full-suite.json').read_text())['exit_code']==0
quality=json.loads((OUT.parent/'v72-candidate-quality/packet.json').read_text());assert quality['engineering_passed'] and quality['source']==revision
audit=json.loads((OUT.parent/'v72-candidate-quality-audit/packet.json').read_text());assert audit['source']==revision and len(audit['quality'])==12
...
p={'scanner':identity,'frozen_checkout':str(fixed),'tracked_clean':True,'untracked_clean':True,'recorded_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'harness_and_lock_sha256':{n:hashlib.sha256((fixed/n).read_bytes()).hexdigest() for n in ['uv.lock','scripts/phase20_corpus.py','scripts/phase22_corpus.py','scripts/phase20_measurements.py','scripts/phase20_scoring.py']},'source_exposure':'All four already-exposed repositories and earlier compatibility inputs retain their original exposure before this correction. Original four fresh gates failed at2e0efb2 and remain failed. Required local and hosted engineering has passed before this freeze; no corrected-scanner corpus observation is authorized or performed.','storage':'Complete clean Git tree with copy-on-write artifact clones; source and tests checked out at actual cec0322.','new_corpus_observations':0,'paid_calls':0}
