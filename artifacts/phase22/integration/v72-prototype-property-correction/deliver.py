"""Deliver verified ordinary correctness and its unapproved regression to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = 'cec0322e904bbf63c33cd95796e7289101a93e5d'
TITLE = 'Phase 22: correctness verified; corrected regression approval pending'
...
assert read(PREV/'optimization-proposal.json')['status']=='prepared_not_approved_not_started'
assert not (BASE/'v69-ts-literal-key-reuse').exists()
seal = read(BASE/'evidence-v98.json')
assert sha(BASE/'evidence-v98.json') == binding['evidence_seal']['index_sha256']
...
for name, digest in read(BASE/'v72-prototype-property-correction/local-checks.json')['candidate_engineering_files_sha256'].items():
    if name != 'README.md': assert sha(ROOT/name) == digest, name
current = pr()
guard(current)
assert current['title'] == 'Phase 22: prototype boundary corrected; full engineering in progress'
assert current['headRefOid'] == EXPECTED and current['body'] == (OUT/'candidate-pr-body.md').read_text()
...
extra = [BASE/'evidence-v98.json', BASE/'evidence-v98.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v72-documentation-binding.*'))
...
save('staged-delivery.json', {'sha256': {n: sha(ROOT/n) for n in sorted(staged)}, 'protected_user_files_not_staged': True, 'pending_optimization_proposal_sha256': binding['pending_optimization_proposal_sha256']})
print(run(['git', 'commit', '-m', 'Record verified correctness recovery and pending corrected regression [skip ci]']), flush=True)
head = run(['git', 'rev-parse', 'HEAD']).strip()
save('delivery-commit.json', {'head': head, 'parent': EXPECTED, 'pending_optimization_proposal_sha256': binding['pending_optimization_proposal_sha256'], 'technical_acceptance_received': False})
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    'body_sha256': sha(OUT/'pr-body-delivery.md'), 'evaluation_proposal_sha256': binding['evaluation_proposal_sha256'], 'evaluation_authorization_sha256': binding['evaluation_authorization_sha256'], 'pending_optimization_proposal_sha256': binding['pending_optimization_proposal_sha256'], 'scanner': binding['corrected_scanner'],
    'seal98_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_native_observations': 0, 'experiment_observations': 0, 'experiment_budget': binding['experiment_budget'], 'current_source_input_accounting': binding['current_source_input_accounting'], 'profiles_in_this_continuation': 0, 'optimization_approved': False, 'experiment_approved': False, 'paid_calls': 0,
    'repository_gates_passed': 0, 'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V72-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only; no technical acceptance.'},
    'effective_additional_counts': {'passed': 291, 'historical_closure_proposals': 6, 'unresolved': 21},
    'provenance': 'Supplemental post-commit readback, outside seal98 and the commit it verifies; retain in next authorized seal.'}
