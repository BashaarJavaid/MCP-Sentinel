...
p['failures_preserved'] += [
 ...
 {'check':'v71-full-suite','outcome':'Deliberately interrupted after1408passed/36skipped in682.16seconds, exit2, once the independent prototype control failed. Initial process-name filter found no lowercase-python match and sent no signal; exact macOS Python PID70409 was then verified and interrupted normally. Partial JUnit/log/coverage remain retained; no complete local pass.'},
...
p['initial_engineering_evidence_sha256']={str(path.relative_to(root)):sha(path) for path in [base/'v71-prototype-property-control.json',base/'v71-prototype-property-control.log',base/'v71-full-suite.json',base/'v71-full-suite-junit.xml',base/'v71-full-suite-coverage.json',base/'v71-candidate-quality/packet.json',prior/'prototype-engineering-stop.json']}
