"""Proposed 24-observation uncapped corrected four-repository regression; separate approval required; identity, output, cleanup and repeat gates retained."""
...
spec_policy = importlib.util.spec_from_file_location("uncapped", ASSETS / "uncapped.py")
uncapped = importlib.util.module_from_spec(spec_policy)
spec_policy.loader.exec_module(uncapped)
...
def verify():
    ...
    assert proposal["bounds"]["native_observations"] == len(proposal["order"]) == 24
    assert len({(x["group"], x["input_id"]) for x in proposal["order"]}) == 12
...
def approved():
    ...
    assert receipt["intake_sha256"] == sha(ASSETS / "intake.json")
...
def identity(group):
    ...
    assert STATIC_TIMEOUT_SECONDS == 1800
...
def child(group, input_id, out):
    ...
    policy_binding = {
        "scanner": proposal["scanner"], "runtime_policy_sha256": sha(ASSETS / "uncapped.py"),
        "scanner_deadline_seconds": None, "semgrep_rule_timeout_seconds": 0,
        "worker_deadline": "positive infinity inherited in the existing private IPC",
        "default_product_deadline_seconds": 1800, "configuration_sha256": expected["configuration_sha256"],
        "imports": {name: str(module.__file__) for name, module in {
            "engine": __import__("sentinel.static.engine", fromlist=["engine"]),
            "workers": __import__("sentinel.static.workers", fromlist=["workers"]),
            "typescript_path_flow": __import__("sentinel.static.typescript_path_flow", fromlist=["typescript_path_flow"]),
        }.items()}, "verified_before_target_access": True,
    }
    assert all(Path(path).is_relative_to(Path(proposal["frozen_checkout"])/"src") for path in policy_binding["imports"].values())
    write(out.with_name(out.name + "-runtime-policy.json"), policy_binding)
    with uncapped.policy(), patch.object(harness, "load_configuration", checked):
...
def run(out):
    ...
    packet = {
        ...
        "execution": "single local serial uncapped experimental sequence",
        "runtime_policy_sha256": sha(ASSETS / "uncapped.py"),
        "whole_input_limit_seconds": None,
        "sequence_limit_seconds": None,
    ...
    try:
        ...
        for n, observation in enumerate(order):
            ...
            with uncapped.unlimited_waits():
                row["timing"] = supervisor.supervise(command, out / (label + ".log"), float("inf"))
        ...
        assert len(packet["attempts"]) == proposal["bounds"]["native_observations"] == 24 and not packet["unstarted"]
        packet["passed"] = True
        packet["detection_and_coverage_gate_passed"] = None
    except BaseException as error:
        if packet["attempts"] and packet["attempts"][-1]["state"] == "attempted":
            packet["attempts"][-1]["state"] = "interrupted_or_failed_before_timing_receipt"
