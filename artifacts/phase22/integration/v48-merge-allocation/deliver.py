"""Deliver the engineering-verified candidate and exact native proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = BASE/'v47-faf-sampling'
EXPECTED = '6e4fd671a97d92f6297dffc900c4dd9e18faad2c'
TITLE = 'Phase 22: allocation candidate verified; regression approval pending'
...
assert not (BASE/'v49-allocation-regression').exists()
seal = read(BASE/'evidence-v76.json')
assert sha(BASE/'evidence-v76.json') == binding['evidence_seal']['index_sha256']
...
for name, digest in read(BASE/'v48-merge-allocation/local-checks.json')['candidate_engineering_files_sha256'].items():
    assert sha(ROOT/name) == digest, name
current = pr()
guard(current)
...
extra = [BASE/'evidence-v76.json', BASE/'evidence-v76.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'assess-source.py']
extra.extend(BASE.glob('v48-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain verified allocation candidate and native regression proposal [skip ci]']), flush=True)
...
print(run(['gh', 'pr', 'edit', '37', '--title', TITLE, '--body-file', str(OUT/'pr-body-delivery.md')]), flush=True)
history = []
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal76_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'native_observations': 0, 'profiles': 0, 'old_budgets_closed': True, 'optimization_attempts': 1, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V48-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 146, 'historical_closure_proposals': 6, 'unresolved': 6},
    'provenance': 'Supplemental post-commit readback, outside seal76 and the commit it verifies; retain in next authorized seal.'}
