...
assert sha(ASSETS/'evaluation-proposal.json') == 'c4186f267057e047c788c1bef0f268f12473ca8adf9140b58b327d080748f078'
assert sha(ASSETS/'launcher-binding.json') == '3daa6cc29b4ab31d1253eda1db7debcbdf1c725e8f78f534d064babab55b6dea'
assert not (ASSETS/'evaluation-authorization.json').exists()
assert not (ASSETS/'execution-consumed.json').exists()
assert not (ASSETS.parent/'v49-allocation-regression').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '129d3462a879e24e02e502d8ee38b950a0293bc7'
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v49-', 'v48-', 'v47-', 'v45-', 'v46-', 'v43-', 'v44-', 'phase22-check.py', 'mcp-phase22-four-fresh-', 'v42-', 'v41-', 'v40-fresh-v7/', 'v38-guard-regression/']
...
receipt = {'recorded_at': datetime.now(timezone.utc).isoformat(), 'approved': True,
           'user_decision': 'approved.', 'decision_context': 'User approved the exact 205-observation proposal and launcher presented after verified draft delivery 129d346. This is evaluation approval only; technical acceptance remains separate.',
