...
quality = read(BASE/'v48-candidate-quality/packet.json')
assert quality['engineering_passed'] and quality['source'] == freeze['scanner']['revision']
assert read(BASE/'v48-full-suite-restored.json')['exit_code'] == 0
...
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': freeze['scanner'],
    ...
    'purpose': 'Measure the approved allocation candidate, the four previously exposed source corrections and every still-unvalidated prior Python/TypeScript input after current engineering verification. Preserve the original four fresh failures and all earlier results.',
    'runner': str((OUT/'evaluate.py').relative_to(ROOT)),
    'prepared_command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'evaluate.py'), 'run', str(BASE/'v49-allocation-regression/raw')],
    ...
    'scope_justification': '24 four-repository observations plus the prior94-observation TypeScript and87-observation Python schedules. This allocation change is TypeScript-only, but the earlier17b4784 source recovery changed both languages and its205-observation sequence stopped before any completed input. The Python87 and TypeScript94 compatibility gates therefore both remain necessary; no closed remainder is reused. This covers110 unique exposed input records and95 ordered pairs, without adding a second Python-development pass or new mutation/repository. It is not a new whole25/45 Linux timing execution.',
