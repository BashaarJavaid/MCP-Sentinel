...
assert not (BASE/'v51-faf-bound-sampling').exists()
seal = read(BASE/'evidence-v78.json')
assert sha(BASE/'evidence-v78.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v78.json', BASE/'evidence-v78.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'diagnostic.py', OUT/'sampling-worker.py', OUT/'corrected-sampling-worker.py', OUT/'sampler-selfcheck.py', OUT/'assess-and-prepare.py']
extra.extend(BASE.glob('v50-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain invalid profile and corrected worker identity proposal [skip ci]']), flush=True)
