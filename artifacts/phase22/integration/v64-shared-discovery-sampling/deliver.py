"""Deliver assessed profile and unapproved If invalidation-accumulator proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = OUT
EXPECTED = '7d6e01a0e8c9e4e5f81a8fd195599dac6b7aa8ab'
TITLE = 'Phase 22: profile assessed; invalidation proposal pending'
...
assert sha(BASE/'evidence-v92.json') == binding['evidence_seal']['index_sha256']
...
current = pr()
guard(current)
assert current['headRefOid'] == EXPECTED and current['body'] == (BASE/'v63-shared-discovery-regression/pr-body-delivery.md').read_text()
...
extra = [BASE/'evidence-v92.json', BASE/'evidence-v92.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v64-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Assess parent profile and propose bounded If invalidation-accumulator [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal92_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_corpus_observations': 1, 'corpus_budget': binding['corpus_budget'], 'profiles': 1, 'diagnostic_approved': True, 'optimization_approved': False, 'profile_budget': binding['profile_budget'], 'sampled_processes': binding['sampled_processes'], 'samples': binding['samples'], 'regression_approved': True, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V64-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 247, 'historical_closure_proposals': 6, 'unresolved': 16},
    'provenance': 'Supplemental post-commit readback, outside seal92 and the commit it verifies; retain in next authorized seal.'}
