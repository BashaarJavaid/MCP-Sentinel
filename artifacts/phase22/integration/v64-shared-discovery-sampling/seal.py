...
def main():
    assert json.loads((BASE / "v64-shared-discovery-sampling/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v64-shared-discovery-sampling/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v64-shared-discovery-sampling/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v65-if-invalidation-accumulator").exists()
    archive, manifest = BASE / "evidence-v92.tar.gz", BASE / "evidence-v92.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v91.json",
        ...
        "status": "One approved parent-and-worker FAF profile at1948bf9 timed out; actual verified partial snapshots retained, no report, cleanup passed, budget closed. All358requirements and earlier failures retained. One source-only If invalidation-accumulator attempt is proposed,unapproved and unimplemented. Product retains2418tests/36skips engineering; Phase22 incomplete with no human technical acceptance.",
        "verification": (
            "All 91 prior archives and every new member verified afte"
