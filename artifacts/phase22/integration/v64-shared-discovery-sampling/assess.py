...
BASE = OUT.parent
ROOT = OUT.parents[3]
PREP = BASE/'v63-shared-discovery-regression'
...
for row in read(OUT/'sample-attribution.json')['rows']:
    ...
    for sample in d['stacks']:
        ...
        if top['filename'].endswith('/json/encoder.py') and any(frame['filename'].endswith('/typescript_path_flow.py') and frame['function'] == 'expression' and frame['line'] == 1052 for frame in stack):
