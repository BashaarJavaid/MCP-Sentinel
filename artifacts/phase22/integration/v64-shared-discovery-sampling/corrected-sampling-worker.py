...
root=Path('/private/tmp/mcp-phase22-frozen-1948bf9');sys.path[:0]=[str(root/'src'),str(root)]
...
EXPECTED_SCANNER = {'revision': '1948bf942babce155b701f5b22ed941eb16d99d0', 'source_sha256': '8f88940210e0f382f8c1cfa1b8af3bf7464ecb891122ae23539de1e4806fc408', 'harness_sha256': '9bfc9229b7ec7d23580c44f2306edb9274bc1d2a3c2261d01db0445b8b480add'}
def verify_scanner():
 from scripts import phase20_measurements as harness
 from sentinel.static import typescript_path_flow, typescript_discovery
 assert harness.ROOT.resolve()==root.resolve(), 'Harness import root mismatch'
 imports={name:str(Path(module.__file__).resolve()) for name,module in [('workers',workers),('typescript_path_flow',typescript_path_flow),('typescript_discovery',typescript_discovery)]}
 assert imports['workers']==str(root/'src/sentinel/static/workers.py'), 'Worker import root mismatch'
 assert imports['typescript_path_flow']==str(root/'src/sentinel/static/typescript_path_flow.py'), 'Flow import root mismatch'
 assert imports['typescript_discovery']==str(root/'src/sentinel/static/typescript_discovery.py'), 'Discovery import root mismatch'
 identity=harness.scanner_identity()
 assert identity==EXPECTED_SCANNER, 'Worker scanner identity mismatch'
 return {'scanner':identity,'imports':imports,'verified_before_timer_and_target_snapshot':True}

if __name__=='__main__':
 identity=verify_scanner()
 if sys.argv[1:]==['check']:
  print(json.dumps(identity,sort_keys=True));raise SystemExit(0)
 (out/(rule+'-worker-identity.json')).write_text(json.dumps(identity,indent=2)+'\n')
