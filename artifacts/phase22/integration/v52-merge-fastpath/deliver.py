"""Deliver the verified source-only attempt and unapproved regression to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = BASE/'v51-faf-bound-sampling'
EXPECTED = 'a36f696c504e4e8d1f90de5dc84e8942d06316ca'
TITLE = 'Phase 22: merge fast path verified; regression approval pending'
...
assert not (BASE/'v53-fastpath-regression').exists()
seal = read(BASE/'evidence-v80.json')
assert sha(BASE/'evidence-v80.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v80.json', BASE/'evidence-v80.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'prepare-regression.py', OUT/'evaluate.py', OUT/'launch.py', OUT/'reconcile.py', OUT/'final-check.py', OUT/'seal.py']
extra.extend(BASE.glob('v52-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Verify merge fast path and prepare bounded exposed regression [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal80_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 1, 'current_source_corpus_observations': 0, 'profiles': 0, 'evaluation_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V52-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 167, 'historical_closure_proposals': 6, 'unresolved': 9},
    'provenance': 'Supplemental post-commit readback, outside seal80 and the commit it verifies; retain in next authorized seal.'}
