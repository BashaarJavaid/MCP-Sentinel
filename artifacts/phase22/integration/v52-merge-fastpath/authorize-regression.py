...
assert sha(ASSETS/'evaluation-proposal.json') == 'd7dbe81c25e0ea4ef5ad60608a1f78ed11408db8853eb8abe55e33e90d89b913'
assert sha(ASSETS/'launcher-binding.json') == 'c4be5f3d82d85c135191a6458d787339016fbe911bc398ecb125cde9b3999001'
assert not (ASSETS/'evaluation-authorization.json').exists()
assert not (ASSETS/'execution-consumed.json').exists()
assert not (ASSETS.parent/'v53-fastpath-regression').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '43b9a85bee190c2625d2ab748cba773f33551b1c'
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v45-', 'v46-', 'v43-', 'v44-', 'phase22-check.py', 'mcp-phase22-four-fresh-', 'v42-', 'v41-', 'v40-fresh-v7/', 'v38-guard-regression/']
...
receipt = {'recorded_at': datetime.now(timezone.utc).isoformat(), 'approved': True,
           'user_decision': 'approve', 'decision_context': 'User approved the exact 205-observation proposal and launcher presented after verified draft delivery 43b9a85. This is evaluation approval only; technical acceptance remains separate.',
