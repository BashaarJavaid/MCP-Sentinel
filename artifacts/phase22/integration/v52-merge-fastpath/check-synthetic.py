...
for count in (1, 2, 3, 8):
    for missing in [None, *range(count)]:
        ...
        expected_old = int(any(branches) and missing is not None)
...
# Count fallback preparation itself, including distinct-but-equal Values.
lookup_rows=[]
for shared in (False, True):
    for count in (1,2,3,8):
        value=Value(key='root',sources=frozenset({'caller'}),contained=True)
        branches=[{'#record:root': value if shared else module.replace(value)} for _ in range(count)]
        calls=[]
        for kernel in (baseline,module.TypeScriptPathFlow.merge):
            f=flow(True,False)
            with patch.object(f,'objects',wraps=f.objects) as objects:
                env={};kernel(f,env,branches)
            assert env=={'#record:root':value} and env['#record:root'] is branches[0]['#record:root']
            calls.append(objects.__contains__.call_count)
        assert calls==[1,0],(shared,count,calls)
        lookup_rows.append({'shared_instance':shared,'branches':count,'baseline_record_registry_lookups':calls[0],'candidate_record_registry_lookups':calls[1]})
packet = {'passed': True, 'semantic_cases': cases, 'equal_environment_and_entire_flow_state': True, 'inputs_unchanged': True, 'allocation_cases': allocations, 'used_fallbacks_preserved': used, 'additional_fallback_allocations': 0, 'fallback_preparation_cases': lookup_rows, 'fallback_preparations_avoided': sum(r['baseline_record_registry_lookups']-r['candidate_record_registry_lookups'] for r in lookup_rows), 'baseline_sha256': hashlib.sha256((out/'baseline-merge.py').read_bytes()).hexdigest(), 'candidate_source_sha256': hashlib.sha256(Path(module.__file__).read_bytes()).hexdigest(), 'corpus_observations': 0, 'profiles': 0, 'paid_calls': 0, 'native_performance_claim': False}
