...
checks=['v52-local-checks','v52-quality-audit','v52-frozen-validation','v52-launcher-check','v52-reconciliation','v52-final-docs','v52-final-build','v52-final-distributions','v52-check-failure-assessment','v52-owned-cleanup']
...
assert not (out/'evaluation-authorization.json').exists() and not (out/'execution-consumed.json').exists() and not (base/'v53-fastpath-regression').exists()
assert read(out/'final-distributions.json')['wheel_product_schema_fixture_members_byte_identical_to_hosted']
audit=read(out/'audit.json');assert len(audit['requirements'])==89 and len(audit['additional_scope_requirements'])==182
assert audit['requirements']==read(base/'v51-faf-bound-sampling/audit.json')['requirements']
...
write('command-provenance.json',{'wrapper_commands':{p.stem:read(p) for p in sorted(base.glob('v52-*.json')) if 'command' in read(p)},'direct_commands':['authorize.py preserved baseline before editing','ruff format tests/test_typescript_discovery.py','Reused evaluate.py byte-identically and changed only supervisor output path','explicit candidate pusha36f696 to existing draft branch, normal CI34767417083/docs34767417081'],'failure_assessment_sha256':sha(out/'check-failure-assessment.json'),'paid_calls':0})
write('final-checks.json',{'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'scanner':p['scanner'],'engineering_files_verified':len(local['candidate_engineering_files_sha256']),'prior_evidence_files_verified':len(prior),'requirements':271,'protected_user_files':8,'owning_docs':16,'main_and_parent_preserved':True,'checks_sha256':{n:sha(base/(n+'.json')) for n in checks},'evaluation_proposal_sha256':sha(out/'evaluation-proposal.json'),'launcher_binding_sha256':sha(out/'launcher-binding.json'),'optimization_attempts':1,'native_observations':0,'profiles':0,'old_budgets_closed':True,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False})
print('Final checks passed:302engineering files,271requirements,16docs,unapproved205-observation proposal.')
