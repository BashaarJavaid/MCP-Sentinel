...
def main():
    assert json.loads((BASE / "v55-singleton-combine/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v55-singleton-combine/owned-work.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v55-singleton-combine/optimization-budget-closed.json").read_text())["closed"]
    assert not (BASE / "v56-canonical-unknown").exists()
    archive, manifest = BASE / "evidence-v83.tar.gz", BASE / "evidence-v83.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v82.json",
        ...
        "status": "One approved broad singleton optimization failed synthetic environment equivalence and was reverted; complete candidate/baseline/failed helper and structural reassessment preserved, real-target reachability unestablished. Initial reconciliation assertion defect retained and corrected. All290requirements retained. Product restored to tested a36f696; final docs/packages pass. Separate canonical UNKNOWN_VALUE-only proposal unapproved. All historical failures/budgets unchanged; Phase22 incomplete and no technical acceptance.",
        "verification": (
            "All82 prior archives and every new member verified afte"
