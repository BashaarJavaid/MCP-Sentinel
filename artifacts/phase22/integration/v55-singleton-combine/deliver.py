...
EXPECTED = '029bef12f71fd507be168fc7731b3b63d20b3ae5'
TITLE = 'Phase 22: singleton attempt reverted; narrower proposal pending'
...
assert not (BASE/'v56-canonical-unknown').exists()
seal = read(BASE/'evidence-v83.json')
assert sha(BASE/'evidence-v83.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v83.json', BASE/'evidence-v83.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'authorize.py', OUT/'baseline-path-flow.py', OUT/'candidate-path-flow.py', OUT/'candidate.patch', OUT/'check-identity.py', OUT/'assess-failure.py', OUT/'reconcile-first.py', OUT/'reconcile.py', OUT/'final-check.py', OUT/'seal.py']
extra.extend(BASE.glob('v55-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain failed singleton attempt and canonical-unknown proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal83_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    ...
    'effective_scope_disposition': {'id': 'V55-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 183, 'historical_closure_proposals': 6, 'unresolved': 12},
    'provenance': 'Supplemental post-commit readback, outside seal83 and the commit it verifies; retain in next authorized seal.'}
