"""Deliver the completed profile and exact source-only proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = '73d7b61129e2e8ee8c49e491e0a436390ae2142c'
TITLE = 'Phase 22: FAF profile assessed; optimization approval pending'
...
assert not (OUT/'optimization-authorization.json').exists() and not (OUT/'optimization-started.json').exists()
assert not (BASE/'v48-merge-allocation').exists()
seal = read(BASE/'evidence-v75.json')
assert sha(BASE/'evidence-v75.json') == binding['evidence_seal']['index_sha256']
...
with tarfile.open(BASE/seal['archive']) as archive:
    assert len(archive.getmembers()) == len(seal['files']) == 90
...
for name, digest in read(BASE/'v45-four-source-recovery/local-checks.json')['candidate_engineering_files_sha256'].items():
    assert sha(ROOT/name) == digest, name
current = pr()
guard(current)
...
extra = [BASE/'evidence-v75.json', BASE/'evidence-v75.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'assess.py']
extra.extend(BASE.glob('v47-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain FAF profile and narrow allocation proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal75_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'sampled_attempts': 1, 'sampled_completed_inputs': 0, 'budget_closed': True, 'remaining': 0, 'optimization_attempts': 0, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V47-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 138, 'historical_closure_proposals': 6, 'unresolved': 6},
    'provenance': 'Supplemental post-commit readback, outside seal75 and the commit it verifies; retain in next authorized seal.'}
