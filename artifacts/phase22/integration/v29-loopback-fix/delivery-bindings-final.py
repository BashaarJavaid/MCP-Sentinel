...
post_audit_document_deltas={}
for name,digest in audit['current_source_and_test_files'].items():
 if sha(r/name)!=digest:
  assert name in names, name
  post_audit_document_deltas[name]={'audit_sha256':digest,'final_sha256':sha(r/name)}
...
p['post_audit_document_deltas']=post_audit_document_deltas
p['audit_document_binding_scope']='Audit snapshot precedes final owning-doc reconciliation. Only the explicitly enumerated owning documents may differ; this final binding is authoritative for their final hashes. Product/test/workflow/package inputs are separately byte-verified against tested439c3fe. The initial binding helper failure and correction are retained in post-seal-binding-correction.json.'
