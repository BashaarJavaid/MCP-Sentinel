...
EXPECTED='2453698eb65eed83b121f2661a4004319617693f';TITLE='Phase 22: unseen results assessed; technical acceptance pending'
...
seal=read(BASE/'evidence-v102.json');assert sha(BASE/'evidence-v102.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: corrected unseen-evaluation proposal pending'
assert current['body']==(BASE/'v79-unseen-metadata-correction/pr-body-delivery.md').read_text()
...
extra=[BASE/'evidence-v102.json',BASE/'evidence-v102.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v81-documentation-binding.*')]
...
print(run(['git','commit','-m','Deliver unseen results and explicit Phase 22 acceptance review [skip ci]']),flush=True)
...
counts={**a['additional_scope_dispositions']};counts['passed']+=2;counts['unresolved']-=2
receipt={'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'head':head,'base':'phase22/description-poisoning','draft':True,'open':True,'title':TITLE,'body_sha256':sha(OUT/'pr-body-delivery.md'),'acceptance_proposal_sha256':b['acceptance_proposal_sha256'],'scanner':b['corrected_scanner'],'seal102_sha256':seal['sha256'],'readback_history_sha256':sha(OUT/'delivery-readback-history.json'),'documentation_binding_sha256':sha(OUT/'documentation-binding.json'),'protected_user_files_main_worktrees_preserved':True,'current_source_observations':205,'new_unseen_completed':12,'new_ordered_pairs_equal':6,'new_repository_gates_passed':1,'new_repository_gates_failed':1,'initial_metadata_attempt_failed':True,'whole_ordered_pairs_equal':95,'repository_gates_passed':4,'prior_language_compatibility_passed':True,'budget_closed':True,'remaining':0,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False,'effective_scope_disposition':{'id':'V81-REVIEW-DELIVERY','disposition':'passed','basis':'Affected checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. This resolves review delivery only; explicit human acceptance and actual accepted-closeout delivery remain.'},'effective_additional_scope_dispositions':counts,'provenance':'Supplemental post-commit readback outside its own seal/commit; preserve in next accepted-closeout seal without infinite self-receipt commits.'}
receipt['effective_additional_resolved_rows']=[{'id':'V77-REVIEW','disposition':'passed','basis':'The newly requested full unseen-result technical review is now actually delivered and verified; no human acceptance inferred.'}]
