...
def main():
    assert json.loads((BASE / "v81-technical-acceptance/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v81-technical-acceptance/owned-final.json").read_text())["owned_processes"] == []
    ...
    assert not json.loads((BASE / "v81-technical-acceptance/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v102.tar.gz", BASE / "evidence-v102.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v101.json",
        ...
        "status": "Requested unseen check complete at cec0322:12observations/6pairs, Taskwarrior narrow shell pass, Proxmox vulnerable read missed and fixed/safe unsupported. All4findings/504diagnostics/12surfaces/18contexts/14comparisons assessed. V78metadata failure remains1incomplete/0reports/11closed. Prior205exposed observations/95pairs and actual unchanged2450/36engineering retained. Complete448-row review and26explicit limitation proposals, including Proxmox failure and ordinaryFAF timing/V68unopened. Human technical acceptance and verified accepted closeout pending. Zero new paid calls.",
        "verification": (
            "All 101 prior archives and every new member verified afte"
