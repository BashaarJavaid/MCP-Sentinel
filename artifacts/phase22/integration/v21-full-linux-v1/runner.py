"""The single approved 25 + 45 + 45 sequence; scanner bytes stay frozen."""
...
BASE = ASSETS.parent
PROPOSAL = BASE / "v20-full-linux-sequence-proposal.json"
APPROVAL = BASE / "v21-full-sequence-authorization.json"
...
def approved():
    ...
    for path, digest in proposal["conditions"]["assessment_files"].items():
        assert sha(ASSETS / Path(path).name) == digest
    return proposal


def corpus(kind, proposal):
    assert kind in {j["name"] for j in proposal["bounds"]["jobs"]}
...
def measure_one(kind, input_id, out):
    ...
    expected = json.loads((ASSETS / "configurations.json").read_text())[input_id]
...
def assess(kind, item, directory, timing, proposal):
    ...
    configs = json.loads((ASSETS / "configurations.json").read_text())
    assert (
    ...
    assessments = json.loads((ASSETS / name).read_text())["assessments"]
...
def run(kind, out, deadline_file, previous):
    proposal = approved()
    binding_hash = sha(ASSETS / "binding.json")
    expected = proposal["manifests"][
    ...
    try:
        ...
        assert packet["metrics"]["candidate"]["completed_detections"] == (
            10 if kind == "development" else 20
        )
        assert packet["metrics"]["false_alarm_conditions"] == (
            2 if kind == "development" else 0
        )
        packet["passed"] = True
