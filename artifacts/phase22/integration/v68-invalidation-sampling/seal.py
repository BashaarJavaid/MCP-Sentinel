...
def main():
    assert json.loads((BASE / "v68-invalidation-sampling/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v68-invalidation-sampling/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v68-invalidation-sampling/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v69-ts-literal-key-reuse").exists()
    archive, manifest = BASE / "evidence-v96.tar.gz", BASE / "evidence-v96.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v95.json",
        ...
        "status": "One approved FAF profile at4145d35 timed out after1800.013225seconds:148001periodic parent samples,no report,cleanup verified,final snapshot/timer restoration unconfirmed,budget closed. Exact missing-file restoration and selective user-authorized Docker cleanup retained. All391requirements and earlier failures preserved. One source-only literal serialization reuse proposal is unapproved and unimplemented. Engineering remains4145d35; Phase22 incomplete; zero new paid calls.",
        "verification": (
            "All 95 prior archives and every new member verified afte"
