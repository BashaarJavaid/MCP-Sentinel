"""Deliver assessed profile, recovery and unapproved source-only optimization to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = OUT
EXPECTED = 'c60b7a858497c355326044b959d9f6ad8d755a80'
TITLE = 'Phase 22: profile assessed; literal serialization proposal pending'
...
assert read(OUT/'optimization-proposal.json')['status']=='prepared_not_approved_not_started'
assert not (BASE/'v69-ts-literal-key-reuse').exists()
seal = read(BASE/'evidence-v96.json')
assert sha(BASE/'evidence-v96.json') == binding['evidence_seal']['index_sha256']
...
current = pr()
guard(current)
assert current['title'] == 'Phase 22: invalidation regression timed out; diagnostic pending'
assert current['headRefOid'] == EXPECTED and current['body'] == (BASE/'v67-invalidation-regression/pr-body-delivery.md').read_text()
...
extra = [BASE/'evidence-v96.json', BASE/'evidence-v96.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v68-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Assess current profile and preserve recovery with source-only proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal96_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_corpus_observations': 1, 'corpus_budget': binding['corpus_budget'], 'profile_budget': binding['profile_budget'], 'profiles': 1, 'diagnostic_approved': True, 'optimization_approved': False, 'regression_approved': True, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V68-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 278, 'historical_closure_proposals': 6, 'unresolved': 18},
    'provenance': 'Supplemental post-commit readback, outside seal96 and the commit it verifies; retain in next authorized seal.'}
