...
def main():
    assert json.loads((BASE / "v91-proxmox-disposition/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v91-proxmox-disposition/owned-final.json").read_text())["owned_processes"] == []
    ...
    assert not json.loads((BASE / "v91-proxmox-disposition/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v105.tar.gz", BASE / "evidence-v105.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v104.json",
        ...
        "status": "V90approved6observations/3ordered pairs complete at5142a3f;0/2vulnerable detections and0/4supported negatives. All6reports equal fully source-assessed failed references; closed budget and cleanup verified. Unchanged2596tests/36skips and12hosted suites/29jobs/docs,sixzero-call requests,19runtimebindings. All486requirements/30unaccepted limitations preserved. Concrete Proxmox unsupported disposition awaits user decision; separate technical acceptance/accepted closeout remain. Zero paid calls.",
        "verification": (
            "All 104 prior archives and every new member verified afte"
