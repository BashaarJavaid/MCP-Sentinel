...
OUT=Path(__file__).resolve().parent;BASE=OUT.parent;ROOT=OUT.parents[3];OLD=BASE/'v89-regression-preparation';RESULTS=BASE/'v90-regression-results';ENG=BASE/'v88-literal-metadata-correction'
...
checks=['v90-authorization-preflight','v90-regression-execution','v90-validate-results','v90-complete-reference-comparison','v90-complete-source-assessment-corrected','v90-completion-ownership','v91-review-reconciliation','v91-final-docs','v91-final-generated','v91-final-build','v91-final-distributions']
for n in checks:assert read(BASE/(n+'.json'))['exit_code']==0,n
p=read(OUT/'decision-proposal.json');a=read(OUT/'audit.json');old=read(OLD/'audit.json');binding=read(OLD/'documentation-binding.json');delivery=read(OLD/'delivery-verification.json');head=delivery['head']
assert run('git','rev-parse','HEAD').strip()==head=='0c8888a3fe89030d02bff32beac32faaf0966e45'
...
rows=a['requirements']+a['additional_scope_requirements'];assert len(a['requirements'])==89 and len(rows)==len(a['current_requirement_interpretations'])==486
...
assert sum(r['disposition']=='proposed documented limitation awaiting decision' for r in rows)==30
assert p['new_native_observations_authorized']==p['new_paid_calls']==0 and not p['limitation_disposition_received']
for n,h in p['review_files_sha256'].items():assert sha(ROOT/n)==h,n
proposal=read(OLD/'evaluation-proposal.json');local=read(ENG/'local-checks.json')
...
markers=['phase22-check.py','v90-','v91-','/launch.py','/evaluate.py','mcp-phase22-four-fresh-','-m pytest']
...
commands={path.stem:read(path) for path in BASE.glob('v9[01]-*.json') if 'exit_code' in read(path)}
...
failures={n:c['exit_code'] for n,c in commands.items() if c['exit_code']};assert failures=={'v90-complete-source-assessment':1}
save('check-failure-assessment.json',{'failed_wrapper_checks':{'v90-complete-source-assessment':{'exit_code':1,'assessment':'Initial helper expected intermediate report/condition field names in the final V87 rows and raised KeyError before output. Corrected exact final report_sha256/narrow_rubric_satisfied binding passes. No extra observation, report edit or budget change.'}},'prior_assessment_sha256':sha(OLD/'check-failure-assessment.json'),'qualification':'Every actual failed check retains command/log/patch/helper archive. Prior failed engineering and source checks remain preserved. No automatic approval-review rejection occurred.','paid_calls':0})
