...
EXPECTED='0c8888a3fe89030d02bff32beac32faaf0966e45';TITLE='Phase 22: Proxmox remains unsupported; failure disposition pending'
...
assert b['baseline_delivery']==EXPECTED and not a['technical_acceptance_received'] and a['current_regression_approved'] and a['current_source_corpus_observations']==6 and not a['current_proxmox_gate_passed']
assert read(BASE/'v90-regression-results/completion-verification.json')['budget_closed'] and not read(OUT/'decision-proposal.json')['limitation_disposition_received']
...
assert sha(OUT/'decision-proposal.json')==b['decision_proposal_sha256']
...
seal=read(BASE/'evidence-v105.json');assert sha(BASE/'evidence-v105.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: metadata correction verified; six-observation approval pending'
assert current['body']==(BASE/'v89-regression-preparation/pr-body-delivery.md').read_text()
...
extra=[BASE/'evidence-v105.json',BASE/'evidence-v105.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v91-documentation-binding.*')]
...
print(run(['git','commit','-m','Deliver completed Proxmox failure assessment and disposition review [skip ci]']),flush=True)
...
receipt={'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'head':head,'base':'phase22/description-poisoning','draft':True,'open':True,'title':TITLE,'body_sha256':sha(OUT/'pr-body-delivery.md'),'decision_proposal_sha256':b['decision_proposal_sha256'],'scanner':b['corrected_scanner'],'seal105_sha256':seal['sha256'],'readback_history_sha256':sha(OUT/'delivery-readback-history.json'),'documentation_binding_sha256':sha(OUT/'documentation-binding.json'),'protected_user_files_main_worktrees_preserved':True,'current_source_observations':6,'entire_ordered_pairs_equal':3,'proposed_observations':0,'failed_gate_disposition_received':False,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False,'effective_scope_disposition':{'id':'V91-REVIEW-DELIVERY','disposition':'passed','basis':'Affected checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body verified. This resolves failed-gate review delivery only; explicit Proxmox disposition, complete technical acceptance proposal, human technical acceptance and actual accepted closeout remain.'},'effective_additional_scope_dispositions':counts,'provenance':'Supplemental actual readback outside its own seal/commit; preserve in next authorized evidence seal without infinite self-receipt commits.'}
