...
assert sha(ASSETS/'diagnostic-proposal.json') == '9d3d83da8e6c3295dd98c9462a3bf9809bd8dbf9da442f27cc698ca7ce90f925'
assert not (ASSETS/'diagnostic-authorization.json').exists()
assert not (ASSETS/'diagnostic-consumed.json').exists()
assert not (ASSETS.parent/'v54-faf-fastpath-sampling').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == 'c33721112efac7d9e776b97db84e2d92359b8001'
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v54-', 'v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v46-', 'v45-', 'v44-', 'v43-', 'phase22-check.py', 'mcp-phase22-four-fresh-']
