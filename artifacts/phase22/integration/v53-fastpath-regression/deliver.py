"""Deliver the stopped regression and unapproved current-source diagnostic to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = '43b9a85bee190c2625d2ab748cba773f33551b1c'
TITLE = 'Phase 22: fast-path timeout retained; diagnostic approval pending'
...
seal = read(BASE/'evidence-v81.json')
assert sha(BASE/'evidence-v81.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v81.json', BASE/'evidence-v81.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'assess-and-prepare.py', OUT/'diagnostic.py', OUT/'sampler-selfcheck.py', OUT/'corrected-sampling-worker.py', OUT/'validate.py', OUT/'reconcile.py', OUT/'final-check.py', OUT/'seal.py']
extra.extend(BASE.glob('v53-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain fast-path timeout and bounded current-source diagnostic proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal81_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_corpus_observations': 1, 'corpus_budget': binding['corpus_budget'], 'profiles': 0, 'diagnostic_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V53-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 172, 'historical_closure_proposals': 6, 'unresolved': 10},
    'provenance': 'Supplemental post-commit readback, outside seal81 and the commit it verifies; retain in next authorized seal.'}
