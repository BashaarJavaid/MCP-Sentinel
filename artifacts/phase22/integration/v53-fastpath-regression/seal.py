...
def main():
    assert json.loads((BASE / "v53-fastpath-regression/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v53-fastpath-regression/owned-work.json").read_text())["owned_processes"] == []
    assert not (BASE / "v53-fastpath-regression/diagnostic-authorization.json").exists()
    assert not (BASE / "v53-fastpath-regression/diagnostic-consumed.json").exists()
    archive, manifest = BASE / "evidence-v81.tar.gz", BASE / "evidence-v81.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v80.json",
        ...
        "status": "Approved 205-observation regression at a36f696 stopped on first FAF vulnerable input: one incomplete timeout, no report, 204 unstarted closed, zero remaining; cleanup verified. All 277 requirements retained. One current-source 300-second sampled diagnostic prepared but unapproved/unexecuted. Product bytes remain engineering-tested a36f696; final docs/packages verified. Prior native/profile/fresh failures and v48 restoration/Docker cleanup preserved. Supplemental v52 delivery and exact new native approval included. Phase22 incomplete; no human acceptance, merge, ready-state, release, outreach or Phase23.",
        "verification": (
            "All80 prior archives and every new member verified afte"
