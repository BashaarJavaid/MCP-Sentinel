...
EXPECTED='09fa6ce8541fde5deb1f45055ac9e6de5c8c391c';TITLE='Phase 22: unseen-source evaluation approval pending'
...
assert not (OUT/'execution-consumed.json').exists() and not (OUT/'evaluation-authorization.json').exists() and not (BASE/'v78-unseen-results').exists()
...
seal=read(BASE/'evidence-v100.json');assert sha(BASE/'evidence-v100.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: source gates passed; technical acceptance pending'
assert current['body']==(BASE/'v76-technical-acceptance/pr-body-delivery.md').read_text()
...
extra=[BASE/'evidence-v100.json',BASE/'evidence-v100.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v77-documentation-binding.*')]
...
print(run(['git','commit','-m','Prepare bounded unseen-source Phase 22 evaluation [skip ci]']),flush=True)
