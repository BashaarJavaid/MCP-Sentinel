...
def main():
    assert json.loads((BASE / "v77-unseen-preparation/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v77-unseen-preparation/owned-final.json").read_text())["owned_processes"] == []
    assert not json.loads((BASE / "v77-unseen-preparation/audit.json").read_text())["new_unseen_scope"]["approved"]
    assert not (BASE / "v77-unseen-preparation/execution-consumed.json").exists()
    assert not (BASE / "v78-unseen-results").exists()
    assert not json.loads((BASE / "v77-unseen-preparation/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v100.tar.gz", BASE / "evidence-v100.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v99.json",
        ...
        "verified_corrected_scanner": "cec0322e904bbf63c33cd95796e7289101a93e5d",
        "status": "Two-repository unseen-source proposal prepared at cec0322: six records, twelve proposed observations, zero approved/executed. Source freeze, novelty, complete pairs, source/rubric/configuration/order and normal 1800/10/15/four-worker bounds retained. Synthetic and affected documentation/package checks pass. All 435 requirements and 23 unaccepted limitations preserved, including ordinary FAF completion and unopened V68. Prior 205 exposed observations/95 pairs retain separate closed budgets and actual engineering bindings. Numerical approval, new source assessment, updated delivered technical review, explicit human acceptance and verified closeout remain. Zero new paid calls.",
        "verification": (
            "All 99 prior archives and every new member verified afte"
