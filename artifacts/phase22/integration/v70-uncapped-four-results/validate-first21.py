"""Validate the completed21-observation prefix while the three FAF repeats continue."""
import hashlib,importlib.util,json,sys
from pathlib import Path
out=Path(__file__).resolve().parent;root=out.parents[3];prep=out.parent/'v69-uncapped-four-repository';dest=out/'first21-assessment';dest.mkdir(exist_ok=False)
...
rows=packet['attempts'][:21];assert len(rows)==21 and all(x['state']=='completed' for x in rows)
first={};checked=[]
for row,expected in zip(rows,p['order'][:21],strict=True):
...
d={'passed':True,'scanner':p['scanner'],'proposal_sha256':sha(prep/'evaluation-proposal.json'),'approval_sha256':sha(prep/'evaluation-authorization.json'),'observations':checked,'completed':21,'entire_ordered_pairs_equal':9,'source_assessment_pending':True,'four_repository_sequence_complete':False,'FAF_excluded_from_this_partial_validation':False,'budget_closed':False,'new_inputs':0,'paid_calls':0,'qualification':'All12distinct inputs have completed within the active24-observation experiment; only the three FAF repeats remain. Nine entire non-FAF pairs agree. This is not a new batch, closed budget or technical acceptance.'}
(dest/'validation.json').write_text(json.dumps(d,indent=2)+'\n');print('Validated21completed outputs,9entire ordered pairs; three FAF repeats and full sequence closure remain pending.')
