...
OUT=Path(__file__).resolve().parent;ROOT=OUT.parents[3];ASSETS=OUT.parent/'v69-uncapped-four-repository';DEST=OUT/'first21-assessment'
sys.path[:0]=[str(ROOT/'src'),str(ROOT)]
from scripts.phase22_corpus import frozen,input_files
read=lambda p:json.loads(p.read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert len(read(OUT/'raw/packet.json')['attempts'])>=21 and all(r['state']=='completed' for r in read(OUT/'raw/packet.json')['attempts'][:21])
p=read(ASSETS/'evaluation-proposal.json');p['repositories']=[{'id':g,**v} for g,v in p['groups'].items() ];items={};sources={};caseof={}
...
contexts={};rows=[]
for path in sorted((OUT/'raw').glob('*/*/report.json')):
    if path.parent.name not in items or int(path.parent.parent.name.split('-',1)[0])>21:continue
    ...
    def context(file,line=None,binding=None):
        ...
        if line:
            ...
            base=binding.rstrip(':') if binding.startswith(('.', '/')) else binding.split('.')[0];assert base,(file,binding)
            occ=[n+1 for n,t in enumerate(lines) if re.search(r'(?<![\w$])'+re.escape(base)+r'(?![\w$])',t)]
            if not occ:
