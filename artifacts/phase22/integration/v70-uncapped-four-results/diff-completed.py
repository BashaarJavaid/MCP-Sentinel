"""Retain complete ordered old/current differences, using only approved exclusions."""
import gzip
import hashlib
...
current = read(DEST / 'inventory.json')
assert read(OUT / 'execution.json')['budget_closed'] and len(current['rows']) == 24
