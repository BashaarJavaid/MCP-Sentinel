"""Deliver the completed uncapped experiment and actual failed gates to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
EXPECTED = '8f4c4486ab9644ace9adac2e3570d2f3d4afe9fb'
TITLE = 'Phase 22: uncapped four-repository results; two gates unresolved'
...
assert sha(BASE/'evidence-v97.json') == binding['evidence_seal']['index_sha256']
...
current = pr()
guard(current)
assert current['title'] == 'Phase 22: profile assessed; literal serialization proposal pending'
assert current['headRefOid'] == EXPECTED and current['body'] == (BASE/'v68-invalidation-sampling/pr-body-delivery.md').read_text()
...
extra = [BASE/'evidence-v97.json', BASE/'evidence-v97.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v70-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Record uncapped four-repository timings and source-assessed failures [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal97_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 0, 'current_source_native_observations': 25, 'experiment_observations': 24, 'experiment_budget': binding['experiment_budget'], 'current_source_input_accounting': binding['current_source_input_accounting'], 'profiles_in_this_continuation': 0, 'optimization_approved': False, 'experiment_approved': True, 'paid_calls': 0,
    'repository_gates_passed': 2, 'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V70-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only; no technical acceptance.'},
    'effective_additional_counts': {'passed': 284, 'historical_closure_proposals': 6, 'unresolved': 20},
    'provenance': 'Supplemental post-commit readback, outside seal97 and the commit it verifies; retain in next authorized seal.'}
