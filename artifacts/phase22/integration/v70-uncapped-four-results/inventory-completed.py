...
OUT=Path(__file__).resolve().parent;ROOT=OUT.parents[3];ASSETS=OUT.parent/'v69-uncapped-four-repository';DEST=OUT/'final-assessment'
sys.path[:0]=[str(ROOT/'src'),str(ROOT)]
from scripts.phase22_corpus import frozen,input_files
read=lambda p:json.loads(p.read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert read(OUT/'execution.json')['completed']==24 and read(OUT/'raw/packet.json')['budget_closed']
