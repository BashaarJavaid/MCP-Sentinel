...
assert not subprocess.check_output(['git','diff','4145d35','--','src','tests','scripts','schemas','pyproject.toml','uv.lock','.github','action.yml','Makefile','LICENSE','CHANGELOG.md'])
tracked=set(subprocess.check_output(['git','ls-tree','-r','--name-only','HEAD'],text=True).splitlines())
hosted=list((BASE/'v66-candidate-quality/artifacts/ci/portunusmcp-sentinel-dist').glob('*.whl'));assert len(hosted)==1
...
for path in sorted((OUT/'final-distributions').iterdir()):
 ...
 for n,source in mapping.items():
  ...
  if source!='README.md':assert subprocess.check_output(['git','show','4145d35:'+source])==members[n]
  checked.append({'member':n,'source_path':source,'sha256':sha(members[n]),'binding':'current status-only documentation' if source=='README.md' else 'exact hosted4145d35 Git blob'})
...
assert len(rows)==2
p={'hosted_source':'4145d3559c5300a5003ca8e3368c8da49fb51f91','scanner':'4145d3559c5300a5003ca8e3368c8da49fb51f91','README_sha256':sha((ROOT/'README.md').read_bytes()),'distributions':rows,'wheel_product_schema_fixture_members_byte_identical_to_hosted':True,'qualification':'Final README status documentation changes package long-description metadata only. Newly built wheel/sdist source members match current files; every non-README source member equals its tested4145d35 Git blob. Hosted installed-wheel matrix, Docker and isolation results retain actual4145d35 artifacts; no new hosted run is claimed for docs-only delivery.'}
