...
with (DEST/'inventory-source-corrected.json').open('x') as f:json.dump({'rows':rows,'contexts':contexts,'source_files':{rev:{name:hashlib.sha256(data).hexdigest() for name,data in files.items()} for rev,files in sources.items()}},f,indent=2);f.write('\n')
