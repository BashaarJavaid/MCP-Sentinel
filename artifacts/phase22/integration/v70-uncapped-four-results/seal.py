...
def main():
    assert json.loads((BASE / "v70-uncapped-four-results/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v70-uncapped-four-results/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v70-uncapped-four-results/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v69-ts-literal-key-reuse").exists()
    archive, manifest = BASE / "evidence-v97.tar.gz", BASE / "evidence-v97.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v96.json",
        ...
        "verified_corrected_scanner": "4145d3559c5300a5003ca8e3368c8da49fb51f91",
        "status": "The sole approved uncapped four-repository experiment at4145d35 completed24observations and12entire ordered pairs with verified cleanup. Two narrow exposed gates pass; Lightning qualification and FAF detection/support fail. All399requirements, every earlier failure and v68status correction remain. The shipped1800-second policy is unchanged; earlier-language compatibility remains pending. The literal serialization optimization is unapproved/unimplemented. Phase22incomplete;zero new paid calls.",
        "verification": (
            "All 96 prior archives and every new member verified afte"
