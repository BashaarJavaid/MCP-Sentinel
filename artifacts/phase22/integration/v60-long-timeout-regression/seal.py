...
def main():
    assert json.loads((BASE / "v60-long-timeout-regression/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v60-long-timeout-regression/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v60-long-timeout-regression/budget-closed.json").read_text())["closed"]
    assert not (BASE / "v61-faf-long-timeout-sampling").exists()
    archive, manifest = BASE / "evidence-v88.tar.gz", BASE / "evidence-v88.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v87.json",
        ...
        "status": "Approved 205-observation regression at 7bf4c6e stops at first FAF input after 1800.021672 seconds without report; one incomplete, 204 unstarted closed, zero remaining. All 329 requirements and prior failures retained. One 1800-second sampled FAF profile is prepared, unapproved and unexecuted. Product/tests retain actual 2403/36 engineering. Phase 22 remains incomplete; no human technical acceptance.",
        "verification": (
            "All 87 prior archives and every new member verified afte"
