...
EXPECTED = '24ff6a0f070ec26757383d7bc84930fcc4a6684f'
TITLE = 'Phase 22: 30-minute regression timed out; profiling approval pending'
...
seal = read(BASE/'evidence-v88.json')
assert sha(BASE/'evidence-v88.json') == binding['evidence_seal']['index_sha256']
...
for name, digest in read(BASE/'v59-timeout-policy/local-checks.json')['candidate_engineering_files_sha256'].items():
    assert sha(ROOT/name) == digest, name
current = pr()
guard(current)
...
extra = [BASE/'evidence-v88.json', BASE/'evidence-v88.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v60-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain 30-minute regression timeout and bounded current-source diagnostic proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal88_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    ...
    'effective_scope_disposition': {'id': 'V60-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 220, 'historical_closure_proposals': 6, 'unresolved': 14},
    'provenance': 'Supplemental post-commit readback, outside seal88 and the commit it verifies; retain in next authorized seal.'}
