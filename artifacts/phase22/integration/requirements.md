...
| R36 | Atlassian lifespan/operator origin, real SDK request, service/client construction and enforcement | SENT-016/shared flow | `734aa8c` restores fixed-source default qualification across prepared HTTP exception paths; 383 affected passes, strict typing and source traces retained. Session/default/auth-origin controls through 7071a26 remain. Final condition/compatibility gates remain open. |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified batch 23 retains 923 added/changed completed files through 734aa8c; 149332686 raw bytes, all hashes read back, prior 22 seals retained. Failed and explicitly stopped runs remain preserved. |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Reviewable local commits through 734aa8c remain on the one integration branch. Earlier drafts, failures, implementation and evidence are preserved; no incremental pushes. |
