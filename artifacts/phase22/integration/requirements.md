...
| R35 | Meta ContextVar/HTTP authentication/operator fallback | SENT-016/shared flow | BaseHTTP dispatch, source module replacement and selected SDK providers are implemented. Earlier a2940e2/d2ba110 five-input runs time out. Grouped startup at `f2ea709` completes the first vulnerable input in 119405 ms, 49 findings; c6234a0 subsequently times out on all five inputs (0/5 completed); all failures retained. Frozen fixed-label configuration conflict awaits a versioned user decision; condition/final gates open |
...
| R71 | Full Ruff/format/strict mypy/lock/schema/native/SARIF/pytest >=80% branch | Makefile/workflows + locked environment | Immutable f2ea709 full milestone: 1775 passed, 36 skipped, 89.11% branch coverage; database/logs retained. Later 0491f32 TypeScript selection 292 passes and c6234a0 context/review 49 passes; strict types/style pass. Final candidate complete quality gate open |
