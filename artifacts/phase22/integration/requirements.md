...
The latest completed historical/full-suite source is `dcb965fa872f3b8c76d6a4e87a32ad23f1d5bb24`. Subsequent `b163d7c` corrects legacy TypeScript source coordinates and requires affected/final verification; the earlier passing gates remain source-specific.
...
skips and 89.47% branch coverage at that immutable checkout. The separate Docker,
installed-package, frozen development/held-out, comparator, paid and hosted gates
remain outstanding. `v2-source-audit-bindings-dcb965f/packet.json` binds all three
...
| R09 | Genuine SDK context vs caller/fake/rebound/local SDK | Shared discovery/context source checks | `5f6bb9a` shares SDK annotation recognition for caller/lifespan state, rejects local/relative impersonation and lexical shadowing, and supports genuine Annotated context. Before-failures, 196 focused and 388 HTTP regressions plus strict typing retained. Final actual-source/hosted verification remains open. |
...
| R17 | Helm original bytes/text secrets and explicit YAML omission | Traversal/config tests | Original-text retention, secret locations and strict ordinary/sidecar YAML audited in `v2-workspace-audit-4114f59/packet.json`. Current dcb965f source audit binding and full suite pass; hosted matrix remains open |
| R18 | Historical atlassian-auth condition | SENT-016 and shared lifecycle/middleware/client flows | Historical gate at dcb965f: all four auth inputs complete twice with stable native reports, two vulnerable condition hits and zero fixed-condition alerts. All six findings per fixed input retain ALLOW_GLOBAL_CRED_FALLBACK qualification. Fresh/reviewed evaluation and human acceptance remain separate. |
| R19 | Historical atlassian-ssrf condition | SENT-015, shared URL flow and enforced returned errors | Historical gate at dcb965f: all four SSRF inputs complete twice, with three actual initial-request sink matches per vulnerable input and none in either fixed input. Source guard/authority assessment is retained; redirects/DNS rebinding and runtime proof are outside this result. |
| R20 | Historical atlassian-upload condition | SENT-012, service factories | Historical gate at dcb965f: all five upload/download inputs complete twice, two vulnerable upload condition hits and zero fixed/safe matches. The earlier fixed-upload-mutation timeout at 8bcaded remains. Separate content reads and operator-policy candidates are source-adjudicated. |
| R21 | Historical excel-boundary condition | SENT-012 | Historical gate at dcb965f: five inputs complete twice, two vulnerable conditions detected and zero named fixed/control alerts. The same three parent warnings remain visible with explicit remote filename-check qualification and separate parent-policy assessment. The earlier three-false-alarm score is preserved; no warning was suppressed. |
| R22 | Historical filesystem-prefix condition | SENT-012, TS normalization/component/physical-parent flow | Historical gate at dcb965f: four inputs complete twice, both vulnerable sibling-prefix conditions detected, zero fixed-condition matches. Eight separate temporary-path candidates retain source assessment and platform uncertainty. |
| R23 | Historical git-arguments condition | SENT-014 | Historical gate at dcb965f: four inputs complete twice, both vulnerable diff/checkout option conditions detected, zero fixed-condition matches. Separate date/commit-filter uncertainty and Git-add false alarms remain in the unrelated ledger. |
| R24 | Historical git-repository condition | SENT-012 | Historical gate at dcb965f: four inputs complete twice, repository-selection condition detected in both vulnerable variants and zero fixed-condition matches. Separate staging findings remain visible and source-adjudicated. |
| R25 | Historical git-staging condition | SENT-012 | Historical gate at dcb965f: four inputs complete twice, both vulnerable staging conditions detected, zero fixed-condition matches. Stable IDs and separate Git option warnings are preserved. |
| R26 | Historical kubernetes-shell condition under unchanged SENT-002 meaning | `5065f16`, `typescript_execution.py`, `test_command_execution.py` | Historical gate at dcb965f: five inputs complete twice, both vulnerable kubectl_get shell conditions detected under unchanged SENT-002 meaning, zero fixed/safe matches. Separate Helm shell/path candidates retain source review and uncertainty. |
| R27 | Historical mobile-output condition | SENT-012 | Historical gate at dcb965f: all five inputs complete twice, both vulnerable inputs have screenshot and recording condition hits, zero fixed/control matches. The four output variants preserve exact native content against 8bcaded while the first-run durations drop from about 85-87 seconds to about 36 seconds; these are local observations. Original timeout remains retained. |
...
| R35 | Meta ContextVar/HTTP authentication/operator fallback | SENT-016/shared flow | At `2837d59`, all five Meta development inputs complete natively in 59.5–68.1 seconds with the original deadline. Frozen fixed-label/configuration decision remains pending; completion is not condition acceptance. Final integrated repeat, unrelated adjudication and reviewed tier remain open. |
| R36 | Atlassian lifespan/operator origin, real SDK request, service/client construction and enforcement | SENT-016/shared flow | `8bcaded` retains selection through returned clients, startup configuration and credential fields; shared service/session construction evaluates arguments once. All six fixed-source SENT-016 findings retain the declared-default qualification. Original failures, 995 shared passes before the final sink-only change, 407 final credential/HTTP passes, strict types/style/docs and source traces are retained. Final all-rule condition/compatibility gates remain open. |
...
| R41 | Helper execution, ineffective auth, unrelated validation, untrusted hashing | Existing correctness tests | Preserved controls pass in the immutable dcb965f full suite; runtime/hosted gates remain separate |
...
| R58 | Legacy 1.3/1.4/1.5/1.6 reports/baselines, honest unknown coverage | Baseline migration/report tests | Existing and added migration controls pass in the immutable dcb965f full suite; installed consumers and hosted matrix remain separate |
...
| R63 | Historical 45-input deterministic repeat pair, 120s deadline | Existing Phase 20 runner | PASS at immutable dcb965f: 45/45 completed twice under unchanged 120s/input, identical stable reports and no model calls. v2-historical-gate-dcb965f binds both measurements and reproducibility. The earlier 8bcaded 43/45 repeat and all other failures remain. Later b163d7c reporting changes require final verification. |
| R64 | All 20 vulnerable condition hits; zero fixed/safe condition alerts | Frozen condition scorer + per-input adjudication | Historical source-condition gate PASS at dcb965f: 20/20 exposed vulnerable conditions detected, zero matched fixed/safe alerts across 25 controls/partners. Correlated mutations, source review and parent-policy qualification are explicit; no fresh accuracy, model retention or human acceptance is implied. |
| R65 | All five Phase 22 development pairs, paired mutations, controls | Frozen Phase 22 runner | At dcb965f all 25 development inputs complete; all ten vulnerable conditions are source-detected. The original Meta fixed labels yield two disputed alerts, pending explicit review. All 554 unmatched instances are source-assessed. Coordinate correction b163d7c requires remeasurement of affected findings and final-source binding |
| R66 | Fresh holdout outside tuning, separate results/no invented threshold | Approved frozen Phase 22 corpus | First evaluation at dcb965f: 10/25 completed, 10 unsupported, 5 incomplete; zero detections among four completed vulnerable variants, six vulnerable variants unavailable. Ten unrelated TS schema warnings expose incorrect coordinates. All five auth-fetch inputs were marked exposed before b163d7c; subsequent results are regressions. A separately approved replacement freeze is required before a new fresh claim for that slot; original source/labels/results remain unchanged |
| R67 | All new/changed unrelated findings adjudicated; historical 70 unchanged | Evidence/adjudication ledger | Historical current-source ledger at dcb965f assesses all 1487 new unrelated instances, including explicit uncertainties and known false alarms; all 70 original instances retain their unadjudicated decisions. Three qualified Excel parent candidates remain visible and their prior failed score is preserved. At dcb965f all 554 development unmatched instances are separately assessed; the ten held-out unrelated schema warnings are assessed with their coordinate defects. Original 70 decisions remain unchanged. Later coordinate deltas require rebinding. |
| R68 | Pinned comparable Semgrep rerun; Snyk/Cisco unmeasured | Existing comparator runner | Pinned Semgrep 1.176 at dcb965f: 45/45 historical inputs complete, with 271 canonical candidates identical to the accepted Phase 20 comparator after only temporary-root/config-prefix normalization. The original zero condition matches and unrelated unadjudicated decisions are preserved. Development completes 25/25 with 220 warnings; held-out completes 20/25 with 856 warnings and five Solver parser-error inputs. All three condition assessments record zero matches, with unrelated source judgments and uncertainty retained. Unchanged comparator/corpus bindings to 6e68331 are recorded separately; old 40/45 and timeouts remain. No Snyk/Cisco measurement. |
...
| R70 | Git mcp==1.29.0 image/environment identity and Docker startup/discovery/campaigns | Authorized Git packet + existing sandbox | All 13 approved 394bb8a Docker campaigns retain 24/80 tested, exit 3 and cleanup; 312 observations without violations and 728 untested. Current dcb965f catalog/campaign/sandbox equivalence passes for all 13, with unchanged runtime/report code and the approved linux/arm64 image digest rechecked. This reuses execution evidence and establishes no defense. |
| R71 | Full Ruff/format/strict mypy/lock/schema/native/SARIF/pytest >=80% branch | Makefile/workflows + locked environment | Immutable dcb965f: 2007 passed, 36 skipped, 89.47% branch coverage in 1213.21 seconds; exact checkout/import and empty final source diff retained. Current 146-file strict typing and Ruff/format checks pass. Later b163d7c passes 16 reproduced coordinate controls, lint and strict typing; the broader immutable regression check passes 300 and fails two: corrected source-range test passes at 6e68331; the unchanged TypeScript smoke replay test still needs a compatible paid capture. Its exact replacement request is retained without a call. The final immutable full suite remains pending. The older waiting downstream quality/install/preparation queues were stopped before checks began. Final schema/lock/audit/build/Docker/installed/hosted gates remain separate. |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified batch 26 retains 1255 added/changed completed files after batch 25; 391561198 raw bytes, every member read back and hashed, all 25 prior seals retained. Active 6e68331/coordinate queue outputs are explicitly excluded until complete. Failed and explicitly stopped runs remain preserved. |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Reviewable local commits through dcb965f remain on the single integration branch. Earlier drafts, failures, implementation and evidence are preserved; no incremental pushes. |
