...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified v10 adds 385 files through `2066ca7`, including auth condition reports/scoring, class/call controls, all failures and diagnostics; every member read back; sealed v1–v9 and prior coverage preserved |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Local commits through `2066ca7`; original dirty work and every later failed regression preserved; class/call tests now pass and are committed; no repeated parent updates/pushes |
