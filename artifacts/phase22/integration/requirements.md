...
| R36 | Atlassian lifespan/operator origin, real SDK request, service/client construction and enforcement | SENT-016/shared flow | `34220b7` records the actual HTTP/lifespan/client condition on all four exposed inputs; 425 affected regressions. cfeee22/fd9320b add session defaults, copy/override semantics, known Basic Auth and aiohttp origin/header constraints; 831 shared and 42 final focused controls pass with strict types/style. Final condition/compatibility gates remain open |
...
| R63 | Historical 45-input deterministic repeat pair, 120s deadline | Existing Phase 20 runner | Immutable 5f6bb9a first run completes 42/45; both upload fixed inputs and Mobile vulnerable mutation time out under competing checks. Retained as failed; final candidate repeat pair remains pending. |
...
| R70 | Git mcp==1.29.0 image/environment identity and Docker startup/discovery/campaigns | Authorized Git packet + existing sandbox | At `394bb8a`, all 13 approved SDK campaigns remeasured: 24/80 tested each, 56 untested each, exit 3 and clean cleanup. Native 1.7/SARIF/console orchestrator consumer passes. 312 no-violation observations and 728 untested attempts are not defense evidence. Final-source byte/behavior compatibility remains to bind. |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified batch 21 retains 194 completed added/changed files through 5f6bb9a, including approved Git campaigns, successful Meta development completion, exact Mobile serial/worker native equivalence, worker lifecycle and SDK context counterexamples. All hashes read back; prior 20 seals retained. |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Local implementation commits through 5f6bb9a on the integration branch; worker and SDK-context changes tested and sealed. All original failures and drafts preserved; no incremental pushes. |
