...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified batch 24 retains 235 added/changed completed files through 8bcaded; 23442399 raw bytes, all hashes read back, prior 23 seals retained. Failed and explicitly stopped runs remain preserved. |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Reviewable local commits through 8bcaded remain on the one integration branch. Earlier drafts, failures, implementation and evidence are preserved; no incremental pushes. |
