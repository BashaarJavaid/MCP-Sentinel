...
| R35 | Meta ContextVar/HTTP authentication/operator fallback | SENT-016/shared flow | `10cc14c` follows the source-defined tool wrapper; `49f36ce` adds request-local ContextVar state. `f3fab7d` supports attached BaseHTTP dispatch; `0d3f9a3` follows actual module callback replacements; `a035585` resolves the later middleware body. `a2940e2` adds bounded source-selected SDK provider attachment/rejection; 794 shared tests pass. Both a2940e2 and d2ba110 native Meta runs time out on all five inputs. A frozen fixed-label configuration conflict awaits a versioned user decision; native completion/condition gate remain open |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified v16 retains 450 completed added/changed evidence files through `0eb39b5`, including native retries, failures, source-path/factory/cache checks and the pending label packet; every member read back against its hash. Earlier seals and coverage preserved |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Local implementation commits through `f58d7d1`; original dirty work and later failures preserved, with numbered evidence seals; no repeated parent updates/pushes |
