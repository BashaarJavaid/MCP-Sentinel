...
| R21 | Historical excel-boundary condition | SENT-012 | First all-rule 8bcaded run completes five Excel inputs and detects both vulnerable conditions, but three fixed/control parent-directory warnings are conservatively false-alarm scored. 6be3344 retains original-path checks by transport while preserving the parent candidate; 996 shared controls pass through dcb965f. Native remeasurement and acceptance remain open. |
...
| R27 | Historical mobile-output condition | SENT-012 | At 8bcaded, all five inputs complete in the first run; both vulnerable inputs have screenshot and recording condition hits, with zero fixed/control matches. The repeat vulnerable original times out. dcb965f removes repeated immutable TypeScript merge work; 996 shared controls pass. Final repeat remains open. |
...
| R63 | Historical 45-input deterministic repeat pair, 120s deadline | Existing Phase 20 runner | The final 7071a26 sequence was stopped before scanning any input after source review found lost auth qualification. Earlier 42/45 failed runs remain retained. 8bcaded corrects prepared-client and downstream credential qualification; the unchanged-source 45-input repeat pair is prepared on its immutable checkout. |
...
| R68 | Pinned comparable Semgrep rerun; Snyk/Cisco unmeasured | Existing comparator runner | Pinned Semgrep 1.176 measurements at 7071a26: 40/45 historical and 25/25 development inputs complete, five historical rule timeouts retained despite exit 0. Fresh comparator holdout remains open; no Snyk/Cisco comparative measurement. |
...
| R70 | Git mcp==1.29.0 image/environment identity and Docker startup/discovery/campaigns | Authorized Git packet + existing sandbox | All 13 approved 394bb8a Docker campaigns retain 24/80 tested, exit 3 and cleanup; 312 observations without violations and 728 untested. v2-git-runtime-compatibility-7071a26 proves exact catalog/campaign/sandbox equivalence and unchanged runtime/report components through 7071a26. The same 13-input binding also passes at 734aa8c; 8bcaded binding remains pending. |
| R71 | Full Ruff/format/strict mypy/lock/schema/native/SARIF/pytest >=80% branch | Makefile/workflows + locked environment | Immutable 7071a26: 1959 passed, 36 skipped, 89.41% branch coverage; Ruff/format, strict types, schemas, lock/export, notices, artifacts, audit/docs/build pass. Later 734aa8c: 383 affected tests and strict types/style/docs pass; final integrated verification remains open. |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified batch 25 retains 396 added/changed completed files through dcb965f; 323987859 raw bytes, every member read back and hashed, prior 24 seals retained. Failed and explicitly stopped runs remain preserved. |
