...
| R11 | uv/npm/pnpm members and exclusions | `workspaces.py`, `test_workspaces.py` | Delivered increment retained; final-source audit open |
| R12 | TS aliases/inherited local compiler configuration, inaccessible parents | TypeScript module resolver | Delivered evidence retained; final-source audit open |
| R13 | Aggregate root vs individual config, nested config disclosure | Config/traversal/workspace inventory | Existing tests; final-source audit open |
| R14 | Per-member counts without shared-file duplication | Static coverage and native report models | New inconsistent-count regressions retained; final-source audit open |
...
| R19 | Historical atlassian-ssrf condition | SENT-015, shared URL flow and enforced returned errors | Post-`1352a9e` source-bound ASGI middleware gives 2 vulnerable/0 fixed matches for the selected real handler, with 606 affected tests passing. Whole-input completion/native pair/final gate open |
| R20 | Historical atlassian-upload condition | SENT-012, service factories | All five inputs exceed 120 seconds at `3a2a276`. Later nested-decorator/request-cache corrections reach the Confluence upload HTTP read with vulnerable/fixed trace distinction; 616 affected tests pass. Native completion and final gate remain open |
...
| R50 | CLI > env > file > default positive settings; rules-only bypass; Action config | `config.py`, `cli.py`, configuration/Action tests | Settings tests pass; actual Action-consumption verification open |
...
| R53 | Unsupported/inconclusive/startup/execution/interruption states; cleanup | Campaign/sandbox tests | Focused controls pass; actual Docker failures must be resolved |
...
| R60 | Console/native/SARIF and every rule-keyed consumer | Runtime/report/orchestrator/scripts | Coordinated migration still under verification |
...
| R71 | Full Ruff/format/strict mypy/lock/schema/native/SARIF/pytest >=80% branch | Makefile/workflows + locked environment | Fixed `4dfd24c` milestone: 1215 passed, 36 skipped, 88.27% branch coverage; retained database/logs; later 425 Python and 366 TypeScript affected passes separate; final candidate full suite open |
...
| R79 | Lossless numbered evidence batch after v3; per-file hash readback/extraction | Integration archives and command wrapper | Verified v11 adds 170 files through `b51aee1`, including native mobile recording/scoring and all shared-flow failures; every member read back; sealed v1–v10 and prior coverage preserved |
...
| R81 | Reviewable local commits on single integration branch; preserve old drafts | Git history and PRs #22–36 | Local commits through `b51aee1`; original dirty work and later failures preserved; ordered argv/recording and class/call changes committed; no repeated parent updates/pushes |
