"""Bounded replacement-v5 evaluation; requires a separate exact approval."""
...
CORPUS = DELIVERY / "artifacts/phase22/corpus-replacement-v5"
PROPOSAL = CORPUS / "evaluation-proposal.json"
...
def stage():
    ...
    expected.update(
        json.loads((CORPUS / "preparation.json").read_text())["prepared_files_sha256"]
    )
    freeze = proposal["pre_curation_freeze"]
    expected[freeze["path"]] = freeze["sha256"]
...
def run(out, deadline_file, rules):
    proposal = approved()
    order = [
        {"batch": b, "input_id": i}
...
if __name__ == "__main__":
    ...
    if mode == "stage":
        ...
        if mode == "selfcheck":
            ...
            run(out, Path(sys.argv[3]), Path(sys.argv[4]).resolve())
