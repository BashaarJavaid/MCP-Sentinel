...
def main():
    ...
    assert set(new["jobs"]) == set(old["jobs"]) | {"phase22-fresh-v5"}
    for name, job in old["jobs"].items():
        ...
        assert "!inputs.phase22_fresh_v5" in updated["if"]
    assert (
        new[True]["workflow_dispatch"]["inputs"]["phase22_fresh_v5"]["default"] is False
    )
    job = new["jobs"]["phase22-fresh-v5"]
    ...
    packet = {
        ...
        "baseline_sha256": sha(ASSETS.parent / "v23-fresh-v4/baseline.json"),
        ...
        "dispatch_binding": (
            "After explicit approval, bind the actual receipt, final runner/workflow "
            "and all supporting assets before the single dispatch. "
