...
assert read(BASE/'v45-full-suite.json')['exit_code'] == 0
...
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': freeze['scanner'],
    ...
    'purpose': 'Measure the four exposed source corrections and every affected prior Python/TypeScript input after current engineering verification. Preserve the original four fresh failures and all earlier results.',
    'runner': str((OUT/'evaluate.py').relative_to(ROOT)),
    'prepared_command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'evaluate.py'), 'run', str(BASE/'v46-four-source-regression/raw')],
    ...
    'scope_justification': '24 four-repository observations plus the prior94-observation TypeScript and87-observation Python schedules. Shared code changed in both languages, so current compatibility cannot reuse the old Python87 unchanged-source proof. This covers110 unique exposed input records and95 ordered pairs, without adding a second Python-development pass or new mutation/repository. It is not a new whole25/45 Linux timing execution.',
