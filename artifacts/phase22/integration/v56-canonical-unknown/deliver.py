"""Deliver the failed/reverted singleton attempt and unapproved narrower optimization to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = BASE/'v55-singleton-combine'
EXPECTED = '8df485e7e2dfbea964f8b71db39403f8f3ce17ad'
TITLE = 'Phase 22: eviction failure retained; credential contract proposal pending'
...
assert not (BASE/'v57-credential-merge').exists()
seal = read(BASE/'evidence-v84.json')
assert sha(BASE/'evidence-v84.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v84.json', BASE/'evidence-v84.tar.gz', OUT/'documentation-binding.json', OUT/'bind-delivery.py', Path(__file__).resolve(), OUT/'pre-delivery-pr37.json', OUT/'authorize.py', OUT/'baseline-path-flow.py', OUT/'candidate-path-flow.py', OUT/'candidate.patch', OUT/'check-synthetic.py', OUT/'assess-source.py', OUT/'reconcile.py', OUT/'final-check.py', OUT/'seal.py']
extra.extend(BASE.glob('v56-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain cache-eviction failure and credential-join revision proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal84_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 1, 'candidate_reverted': True, 'optimization_budget': binding['optimization_budget'], 'profiles': 0, 'corpus_observations': 0, 'next_optimization_approved': False, 'next_optimization_attempts': 0, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V56-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 189, 'historical_closure_proposals': 6, 'unresolved': 13},
    'provenance': 'Supplemental post-commit readback, outside seal84 and the commit it verifies; retain in next authorized seal.'}
