...
def main():
    assert json.loads((BASE / "v56-canonical-unknown/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v56-canonical-unknown/owned-work.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v56-canonical-unknown/optimization-budget-closed.json").read_text())["closed"]
    assert not (BASE / "v57-credential-merge").exists()
    archive, manifest = BASE / "evidence-v84.tar.gz", BASE / "evidence-v84.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v83.json",
        ...
        "status": "Canonical UNKNOWN_VALUE bypass passes two ordinary identity cases then fails cache-eviction environment equivalence; full flow equal, target impact unestablished. Candidate preserved and reverted; one attempt closed. All297requirements retained; tested a36f696 engineering/source restored and final docs/packages pass. Prospective credential-marker equality contract revision plus canonical bypass unapproved. Both singleton failures and all original fresh/native/profile failures stay source-bound. Phase22 incomplete, no technical acceptance.",
        "verification": (
            "All83 prior archives and every new member verified afte"
