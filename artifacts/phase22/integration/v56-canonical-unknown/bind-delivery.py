...
seal=read(BASE/'evidence-v84.json');assert sha(BASE/seal['archive'])==seal['sha256']
save('seal-execution.json',{'command':['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python','artifacts/phase22/integration/v56-canonical-unknown/seal.py'],'exit_code':0,'members':len(seal['files']),'raw_bytes':sum(r['bytes'] for r in seal['files']),'archive_sha256':seal['sha256'],'qualification':'Direct seal execution after writers stopped. Supplemental completion receipt.'})
path=BASE/'README.md';text=path.read_text().replace('batches 1–83','batches 1–84',1);first,rest=text.split('\n',1)
block=f'''
## Failed canonical-bypass evidence seal 84

[Seal 84](evidence-v84.json) retains **{len(seal['files'])} members / {sum(r['bytes'] for r in seal['files']):,} raw bytes**;
archive SHA-256 `{seal['sha256']}`. All 83 preceding archives and every member
...
body=(OUT/'pr-body.md').read_text()+f'\n[Seal 84]({url}evidence-v84.json) retains {len(seal["files"])} members / {sum(r["bytes"] for r in seal["files"]):,} raw bytes; SHA-256 `{seal["sha256"]}`. All 83 earlier archives verified. Final docs/packages pass with restored code identical to hosted `a36f696`; post-seal readback is supplemental.\n'
