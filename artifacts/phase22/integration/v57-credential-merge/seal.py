...
def main():
    assert json.loads((BASE / "v57-credential-merge/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v57-credential-merge/owned-work.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v57-credential-merge/optimization-budget-closed.json").read_text())["closed"]
    assert not (BASE / "v58-credential-regression").exists()
    archive, manifest = BASE / "evidence-v85.tar.gz", BASE / "evidence-v85.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v84.json",
        ...
        "verified_corrected_scanner": "dc7371513a065457af566f4b589b9ed147130d64",
        "status": "Approved equal-marker credential join and canonical UNKNOWN_VALUE bypass pass source assessment, synthetic comparisons, local and hosted engineering at dc73715. All 308 requirements retained. A separate 205-observation exposed regression is prepared, unapproved and unexecuted. Both singleton failures and all earlier fresh/native/profile failures remain source-bound and unaccepted. Phase 22 remains incomplete; no technical acceptance.",
        "verification": (
            "All 84 prior archives and every new member verified afte"
