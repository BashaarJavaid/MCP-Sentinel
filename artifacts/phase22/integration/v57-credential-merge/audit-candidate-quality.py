...
source = b / "v57-candidate-quality"
...
p = {
    ...
    "classification": "Fresh current-source engineering for the approved credential-join correction and canonical-bypass candidate dc73715. All four optional corpus jobs skipped. Retained historical reproductions use8824014; no fresh or exposed corrected corpus observation is inferred.",
}
local_coverage = json.loads((b / "v57-full-suite-coverage.json").read_text())["totals"]
...
assert len(p["quality"]) == 12 and all(
    r["passed"] == 2399 and r["skipped"] == 36 for r in p["quality"]
)
tracked = set(
...
checkout = json.loads((b / "v57-credential-merge/checkout-binding.json").read_text())
...
p["checkout_binding_sha256"] = sha((b / "v57-credential-merge/checkout-binding.json").read_bytes())
p["actual_synthetic_merge"] = checkout["synthetic_merge"]
p["complete_checkout_tree_equals_head"] = True
out = b / "v57-candidate-quality-audit"
