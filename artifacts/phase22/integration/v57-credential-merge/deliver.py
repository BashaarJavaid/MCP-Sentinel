"""Deliver the verified credential correction and unapproved regression proposal to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = OUT
EXPECTED = 'dc7371513a065457af566f4b589b9ed147130d64'
TITLE = 'Phase 22: credential correction verified; regression approval pending'
...
assert read(OUT/'optimization-budget-closed.json')['closed'] and not (OUT/'next-authorization.json').exists()
assert not (BASE/'v58-credential-regression').exists()
assert not (OUT/'evaluation-authorization.json').exists() and not (OUT/'execution-consumed.json').exists()
seal = read(BASE/'evidence-v85.json')
assert sha(BASE/'evidence-v85.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v85.json', BASE/'evidence-v85.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v57-documentation-binding.*'))
...
save('staged-delivery.json', {'sha256': {n: sha(ROOT/n) for n in sorted(staged)}, 'protected_user_files_not_staged': True, 'evaluation_proposal_sha256': binding['evaluation_proposal_sha256']})
print(run(['git', 'commit', '-m', 'Verify credential correction and prepare exposed regression proposal [skip ci]']), flush=True)
head = run(['git', 'rev-parse', 'HEAD']).strip()
save('delivery-commit.json', {'head': head, 'parent': EXPECTED, 'evaluation_proposal_sha256': binding['evaluation_proposal_sha256'], 'technical_acceptance_received': False})
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    'body_sha256': sha(OUT/'pr-body-delivery.md'), 'evaluation_proposal_sha256': binding['evaluation_proposal_sha256'], 'scanner': binding['corrected_scanner'],
    'seal85_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 1, 'candidate_reverted': False, 'optimization_budget': binding['optimization_budget'], 'profiles': 0, 'corpus_observations': 0, 'regression_approved': False, 'regression_observations': 0, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V57-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 200, 'historical_closure_proposals': 6, 'unresolved': 13},
    'provenance': 'Supplemental post-commit readback, outside seal85 and the commit it verifies; retain in next authorized seal.'}
