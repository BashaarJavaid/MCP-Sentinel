...
quality = read(BASE/'v57-candidate-quality/packet.json')
assert quality['engineering_passed'] and quality['source'] == freeze['scanner']['revision']
assert read(BASE/'v57-full-suite.json')['exit_code'] == 0
...
for name in ['authorization.json', 'freeze.json', 'source-recovery-assessment.json', 'compatibility.json',
             'prepare-regression.py', 'evaluate.py', 'scoring-rubric.json', 'correction-validation.json', 'synthetic-validation.json', 'synthetic-reports.json']:
    bind(OUT/name)
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': freeze['scanner'],
    ...
    'purpose': 'Measure the approved credential-join semantic correction and canonical UNKNOWN_VALUE bypass, the earlier exposed four-source recovery and still-unvalidated Python/TypeScript compatibility after engineering and freeze. Preserve all first-frozen failures and both rejected singleton candidates.',
    'runner': str((OUT/'evaluate.py').relative_to(ROOT)),
    'prepared_command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'evaluate.py'), 'run', str(BASE/'v58-credential-regression/raw')],
    'prepared_command_cwd': freeze['frozen_checkout'],
    'launch_precondition': 'A new explicit205-observation authorization, immutable source/configuration/runner binding, one consumed launch token, no conflicting owned work, exact locked interpreter/import paths and an external1150-minute sequence supervisor. Internal stop is1149 minutes, with at most15 seconds process cleanup. No launch receipt or output directory is created by preparation.',
    ...
    'bounds': {'native_observations': 205, 'local_sequences': 1, 'hosted_dispatches': 0,
        'normal_target_seconds': 120, 'native_and_whole_input_maximum_seconds': 300,
        'cleanup_maximum_seconds_per_input': 15, 'sequence_maximum_minutes': 1150, 'internal_stop_minutes': 1149,
        'nominal_input_minutes': 1025, 'retries': 0, 'profiles': 0, 'comparators': 0, 'paid_calls': 0,
    ...
    'scope_justification': '24four-repository observations plus94TypeScript and87Python observations,110unique exposed records and95ordered pairs. Shared combine affects both languages; CredentialFlow changes the specified Python equal-marker join. Earlier source recovery also changed both languages. All three native205-observation attempts stopped on first FAF without a completed report and closed204unstarted each. Both rejected singleton attempts are separately closed. No remainder is reused, no extra Python-development repeat, new mutation or repository is added, and no new whole25/45Linux timing execution is claimed.',
    'correction_contract': {'proposal_sha256': '12eea305c03328d6872587a051c090daf6f2d1bb83e1d0a622e3c3a92af92921', 'authorization_sha256': sha(OUT/'authorization.json'), 'semantic_delta': 'Equal immutable credential markers join independently of object identity.4050synthetic cases retain552explicit marker deltas and552derived cache-key deltas relative to a36f696, with unchanged credential guard results. This is an approved semantic correction, not a claim of complete old-state equivalence. Canonical bypass separately passes2430full-state comparisons and two evictions against the correction-only reference. Seven synthetic HTTP cases preserve entire ordered rule output. All new report deltas require source assessment; no old failure is accepted.'},
