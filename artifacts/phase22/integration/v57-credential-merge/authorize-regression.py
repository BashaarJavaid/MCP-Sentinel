...
assert sha(ASSETS/'evaluation-proposal.json') == '987379ede197b27da159fc0ef11eca6782c0535a66bf8fba061ffe1786e5c03a'
assert sha(ASSETS/'launcher-binding.json') == '3c6878877b27d2fc8aaa91ced4bbd32bc03a4532fafd7c6ed4120400802345ef'
assert not (ASSETS/'evaluation-authorization.json').exists()
assert not (ASSETS/'execution-consumed.json').exists()
assert not (ASSETS.parent/'v58-credential-regression').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == '53d3045b3927ffe3163f4016b98f563b2ed87096'
assert not subprocess.check_output(['git', 'diff', 'HEAD'], cwd=ROOT)
delivery = read(ASSETS/'delivery-verification.json')
assert delivery['passed'] and delivery['head'] == '53d3045b3927ffe3163f4016b98f563b2ed87096' and delivery['evaluation_proposal_sha256'] == sha(ASSETS/'evaluation-proposal.json')
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v58-', 'v57-', 'v56-', 'v55-', 'v54-', 'v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v45-', 'v46-', 'v43-', 'v44-', 'phase22-check.py', 'mcp-phase22-four-fresh-', 'v42-', 'v41-', 'v40-fresh-v7/', 'v38-guard-regression/']
...
receipt = {'recorded_at': datetime.now(timezone.utc).isoformat(), 'approved': True,
           'user_decision': 'approved', 'decision_context': 'User approved the exact 205-observation proposal and launcher presented after verified draft delivery 53d3045. This is evaluation approval only; technical acceptance remains separate.',
