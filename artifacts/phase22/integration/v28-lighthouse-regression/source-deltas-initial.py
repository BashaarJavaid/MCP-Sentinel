...
for a in packet['attempts']:
 ...
 assert {d['pointer'] for d in deltas} <= {'/findings','/warnings','/static_analysis/coverage/surfaces','/static_analysis/coverage/unresolved_flows'}
 rows.append({'input_id':a['input_id'],'before_sha256':sha(old),'after_sha256':sha(p),'deltas':deltas,'assessment':'The constructor correction exposes the handler/sink path: one SENT-015 finding, two unresolved dispatch surfaces and additional rule-specific flow/binding diagnostics. All current occurrences are individually source-assessed in source-assessment.json. Former class-rejection and old binding diagnostics remain in original reports; the new report does not establish complete dispatch or runtime protection.'})
path=OUT/'retained-source-deltas.json';assert not path.exists();path.write_text(json.dumps({'baseline_scanner':'1f3f72f','candidate_scanner':'4a2359d','volatile_exclusions':m.prior.EXCLUDED,'rows':rows,'new_scans':0,'new_paid_calls':0},indent=2)+'\n');print([(r['input_id'],[d['pointer'] for d in r['deltas']]) for r in rows])
