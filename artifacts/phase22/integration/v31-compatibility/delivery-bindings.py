...
assert json.loads((BASE/'v31-final-docs.json').read_text())['exit_code']==0
assert json.loads((OUT/'owned-work-final.json').read_text())['owned_processes']==[]
audit=json.loads((OUT/'audit.json').read_text());assert audit['acceptance_packet_ready'] and audit['current_compatibility_passed']
...
seal=json.loads((BASE/'evidence-v58.json').read_text());assert sha(BASE/seal['archive'])==seal['sha256']
proposal=OUT/'acceptance-proposal.json'
assert not (OUT/'technical-acceptance.json').exists()
refs=['audit.json','assessment.json','source-assessment.json','summary.md','pr-body.md','validation.json','acceptance-proposal.json']
p={'recorded_at':datetime.now(timezone.utc).isoformat(),'tested_workflow':quality,'scanner':audit['scanner_identity'],'lighthouse_evaluation_workflow':'75142f053ab5017ebd0a7f0953994ee9f7c14dd8','lighthouse_run':34648036083,'lighthouse_observations':10,'compatibility_runner_revision':'735e24eb070f203a1a3af337537586109e186d43','compatibility_observations':54,'current_source_corpus_observations':64,'all_current_technical_gates_passed':True,'byte_identical_quality_inputs':inputs,'owning_docs_sha256':{n:sha(ROOT/n) for n in names},'review_packet_sha256':{n:sha(OUT/n) for n in refs},'post_audit_document_deltas':deltas,'evidence_seal':{'path':str(BASE/seal['archive']),'sha256':seal['sha256']},'user_files_sha256':protected,'clean_worktrees':worktrees,'acceptance_proposal_sha256':sha(proposal),'technical_acceptance_pending':True,'new_paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False,'qualification':'Supplemental post-seal document binding. Final remote delivery is verified after push; this record does not claim its future commit or inclusion inside seal58.'}
