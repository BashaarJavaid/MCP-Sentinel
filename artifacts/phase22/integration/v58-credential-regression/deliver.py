...
EXPECTED = '53d3045b3927ffe3163f4016b98f563b2ed87096'
TITLE = 'Phase 22: credential regression timed out; diagnostic approval pending'
...
seal = read(BASE/'evidence-v86.json')
assert sha(BASE/'evidence-v86.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v86.json', BASE/'evidence-v86.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v58-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Retain credential regression timeout and bounded current-source diagnostic proposal [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal86_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    ...
    'effective_scope_disposition': {'id': 'V58-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 206, 'historical_closure_proposals': 6, 'unresolved': 13},
    'provenance': 'Supplemental post-commit readback, outside seal86 and the commit it verifies; retain in next authorized seal.'}
