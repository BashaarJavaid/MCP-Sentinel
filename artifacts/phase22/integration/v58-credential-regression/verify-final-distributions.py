...
assert not subprocess.check_output(['git','diff','dc73715','--','src','tests','scripts','schemas','pyproject.toml','uv.lock','.github','action.yml','Makefile','LICENSE','CHANGELOG.md'])
tracked=set(subprocess.check_output(['git','ls-tree','-r','--name-only','HEAD'],text=True).splitlines())
hosted=list((BASE/'v57-candidate-quality/artifacts/ci/portunusmcp-sentinel-dist').glob('*.whl'));assert len(hosted)==1
...
for path in sorted((OUT/'final-distributions').iterdir()):
 ...
 for n,source in mapping.items():
  ...
  if source!='README.md':assert subprocess.check_output(['git','show','dc73715:'+source])==members[n]
  checked.append({'member':n,'source_path':source,'sha256':sha(members[n]),'binding':'current status-only documentation' if source=='README.md' else 'exact hosteddc73715 Git blob'})
...
assert len(rows)==2
p={'hosted_source':'dc7371513a065457af566f4b589b9ed147130d64','scanner':'dc7371513a065457af566f4b589b9ed147130d64','README_sha256':sha((ROOT/'README.md').read_bytes()),'distributions':rows,'wheel_product_schema_fixture_members_byte_identical_to_hosted':True,'qualification':'Final README status documentation changes package long-description metadata only. Newly built wheel/sdist source members match current files; every non-README source member equals its testeddc73715 Git blob. Hosted installed-wheel matrix, Docker and isolation results retain actualdc73715 artifacts; no new hosted run is claimed for docs-only delivery.'}
