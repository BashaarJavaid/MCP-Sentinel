...
assert sha(ASSETS/'evaluation-proposal.json') == 'a925c4e396b328e2c49f84669fb710f09c58f3490956d868e70a58786ae3c2bf'
assert sha(ASSETS/'launcher-binding.json') == '3d79ee24b85912d4a52c6581f129e133e61f8e1d3755d9b366dd082138ee7de3'
assert not (ASSETS/'evaluation-authorization.json').exists()
assert not (ASSETS/'execution-consumed.json').exists()
assert not (ASSETS.parent/'v67-invalidation-regression').exists()
assert subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip() == 'da7557076b122611dbfedd3587450796073af288'
assert not subprocess.check_output(['git', 'diff', 'HEAD'], cwd=ROOT)
delivery = read(ASSETS/'delivery-verification.json')
assert delivery['passed'] and delivery['head'] == 'da7557076b122611dbfedd3587450796073af288' and delivery['evaluation_proposal_sha256'] == sha(ASSETS/'evaluation-proposal.json')
...
while pid in rows and pid not in ancestors:
    ancestors.add(pid)
    pid = rows[pid][0]
markers = ['v67-', 'v66-', 'v65-', 'v64-', 'v63-', 'v62-', 'v61-', 'v60-', 'v59-', 'v58-', 'v57-', 'v56-', 'v55-', 'v54-', 'v53-', 'v52-', 'v51-', 'v50-', 'v49-', 'v48-', 'v47-', 'v45-', 'v46-', 'v43-', 'v44-', 'phase22-check.py', 'mcp-phase22-four-fresh-', 'v42-', 'v41-', 'v40-fresh-v7/', 'v38-guard-regression/']
