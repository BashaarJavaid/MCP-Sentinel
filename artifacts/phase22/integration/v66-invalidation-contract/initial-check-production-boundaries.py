...
import ast,copy,dataclasses,hashlib,itertools,json,tempfile,types
...
with tempfile.TemporaryDirectory(prefix='v66-owned-ast-') as tmp:
 path=Path(tmp)/'server.ts';source='if (unknown()) { unknown(left); } else { unknown(right); }';path.write_text(source);file=TypeScriptSourceFile(path,'server.ts',source);parsed=TypeScriptProgram((file,),deadline=1e99);node=parsed.trees['server.ts']['Pr'][0]
