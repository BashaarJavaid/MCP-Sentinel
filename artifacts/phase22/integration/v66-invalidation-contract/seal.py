...
def main():
    assert json.loads((BASE / "v66-invalidation-contract/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v66-invalidation-contract/owned-final.json").read_text())["owned_processes"] == []
    assert json.loads((BASE / "v66-invalidation-contract/audit.json").read_text())["budget_closed"]
    assert not (BASE / "v67-invalidation-regression").exists()
    archive, manifest = BASE / "evidence-v94.tar.gz", BASE / "evidence-v94.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v93.json",
        ...
        "status": "One expressly revised private invalidation contract and source-only attempt pass full-state and local/hosted engineering at4145d35. The v65 strict failure and both exact negative-control deltas remain retained. All376requirements retained. Exact205-observation regression prepared, unapproved and unexecuted. Phase22 incomplete; zero new corpus/profile/paid calls.",
        "verification": (
            "All 93 prior archives and every new member verified afte"
