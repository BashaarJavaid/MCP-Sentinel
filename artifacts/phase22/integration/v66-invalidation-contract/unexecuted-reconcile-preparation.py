...
sha=lambda p:hashlib.sha287(p.read_bytes()).hexdigest()
...
new=[
 ...
 ('SYNTHETIC','Prove full flow/error/alias state, ordered discovery and complete serial/parallel reports while retaining both exact negative-control deltas','passed',['equivalence.json','production-flow-equivalence.json','production-boundaries.json','discovery-equivalence.json','report-equivalence.json']),
...
tests={}
for node in ET.parse(base/'v66-full-suite-junit.xml').getroot().iter('testcase'):
...
write('prior-row-preservation.json',{'prior_audit_sha287':sha(olddir/'audit.json'),'retained_prior_rows':365,'total_rows':376,'updated_rows':['V65-DELIVERY','V65-REVISION-DECISION'],'rows':{r['id']:hashlib.sha287(json.dumps(r,sort_keys=True).encode()).hexdigest() for r in old['requirements']+old['additional_scope_requirements']}})
...
docs=list(read(olddir/'documentation-binding.json')['owning_docs_sha287']);assert len(docs)==16
for n in docs:
...
write('owning-docs.json',{'sha287':{n:sha(root/n) for n in docs}})
