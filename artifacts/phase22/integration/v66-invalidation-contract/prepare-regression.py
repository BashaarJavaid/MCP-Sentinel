...
quality = read(BASE/'v66-candidate-quality/packet.json')
...
bind(BASE/'v59-timeout-policy/timeout-confirmation.json')
for name in ['authorization.json', 'freeze.json', 'source-proof.json', 'compatibility.json',
             'prepare-regression.py', 'evaluate.py', 'scoring-rubric.json', 'runner-boundaries.json']:
    bind(OUT/name)
proposal = {'status': 'prepared_not_approved_not_executed', 'scanner': freeze['scanner'],
    ...
    'purpose': 'Evaluate the approved private invalidation-accumulator candidate and still-unvalidated four-source recovery with TypeScript/Python compatibility. The approved 1800-second shared policy is unchanged. All rule-specific analysis executes. Synthetic avoided work proves no native speedup or completion. Preserve all earlier source-bound fresh, native, profile and singleton/strict-accumulator failures.',
    'runner': str((OUT/'evaluate.py').relative_to(ROOT)),
    'prepared_command': ['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python', '-I', str(OUT/'evaluate.py'), 'run', str(BASE/'v67-invalidation-regression/raw')],
    ...
    'scope_justification': '24 original four-repository observations plus 94 TypeScript and 87 Python observations, 110 unique exposed records and 95 ordered pairs. The existing worker orchestration is shared by both languages and mixed workspaces; earlier shared source recovery and credential fixes still lack current-source corpus compatibility because all six prior native regression sequences stopped on their first FAF input. Every old remainder is closed. No extra Python-development repeat, mutation, repository or comparator is added. Whole language subsets are never relabeled as a new whole 25/45 Linux execution.',
    'retained_timeout_policy': copy.deepcopy(read(BASE/'v59-timeout-policy/evaluation-proposal.json')['timeout_policy_revision']),
