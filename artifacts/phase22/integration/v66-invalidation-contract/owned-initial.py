...
p=Path(__file__).with_name('owned-initial.json');assert not p.exists();p.write_text(json.dumps({'recorded_at':datetime.now(timezone.utc).isoformat(),'owned_processes':owned,'managed_container_ids':managed,'sentinel_named_container_ids':named,'qualification':'Read-only check before the approved source-only optimization attempt. The actual attempt count and closed budget are in validation.json. No additional profile, retry or paid call was added. All prior native/profile budgets remain closed. Current process/ancestors excluded. No unrelated work was terminated.'},indent=2)+'\n');print('No remaining owned processes or containers.')
