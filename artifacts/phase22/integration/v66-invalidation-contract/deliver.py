"""Deliver verified invalidation contract and unapproved regression to the existing draft only."""
import hashlib
import json
import subprocess
import tarfile
...
BASE = OUT.parent
OLD = OUT
EXPECTED = '4145d3559c5300a5003ca8e3368c8da49fb51f91'
TITLE = 'Phase 22: invalidation accumulator verified; regression pending'
...
assert not (BASE/'v67-invalidation-regression').exists()
seal = read(BASE/'evidence-v94.json')
assert sha(BASE/'evidence-v94.json') == binding['evidence_seal']['index_sha256']
...
extra = [BASE/'evidence-v94.json', BASE/'evidence-v94.tar.gz', OUT/'documentation-binding.json', OUT/'pre-delivery-pr37.json']
extra.extend(OUT.glob('*.py'))
extra.extend(BASE.glob('v66-documentation-binding.*'))
...
print(run(['git', 'commit', '-m', 'Verify invalidation contract and prepare affected regression [skip ci]']), flush=True)
...
receipt = {'passed': True, 'recorded_at': datetime.now(timezone.utc).isoformat(), 'head': head, 'base': 'phase22/description-poisoning', 'draft': True, 'open': True, 'title': TITLE,
    ...
    'seal94_sha256': seal['sha256'], 'readback_history_sha256': sha(OUT/'delivery-readback-history.json'), 'documentation_binding_sha256': sha(OUT/'documentation-binding.json'),
    'protected_user_files_and_main_preserved': True, 'optimization_attempts': 1, 'current_source_corpus_observations': 0, 'corpus_budget': binding['corpus_budget'], 'new_corpus_observations': 0, 'profiles': 0, 'source_only_attempt_passed': True, 'prospective_contract_revision_approved': True, 'source_only_budget': binding['source_only_budget'], 'historical_profile_budget': binding['historical_profile_budget'], 'regression_approved': False, 'paid_calls': 0,
    'technical_acceptance_received': False, 'phase22_complete': False,
    'effective_scope_disposition': {'id': 'V66-DELIVERY', 'disposition': 'passed', 'basis': 'Final docs/package/source checks, seal, commit/push and exact remote branch/PR head/base/draft/title/body readback verified. Supersedes sealed pending row for delivery only.'},
    'effective_additional_counts': {'passed': 264, 'historical_closure_proposals': 6, 'unresolved': 17},
    'provenance': 'Supplemental post-commit readback, outside seal94 and the commit it verifies; retain in next authorized seal.'}
