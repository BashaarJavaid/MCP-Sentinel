...
for i,(config,actual) in enumerate(observations):
 ...
 record={'index':i,'selected_rules':list(config.scanner.scanner.rules),'baseline':serialize(expected),'candidate':serialize(actual)}
 (OUT/f'synthetic-five-rule-report-{i}.json').write_text(json.dumps(record,indent=2)+'\n')
 assert stable(record['baseline'])==stable(record['candidate']),i
 records.append({'index':i,'selected_rules':list(config.scanner.scanner.rules),'equivalent':True,'finding_count':len(actual.findings),'sha256':hashlib.sha256((OUT/f'synthetic-five-rule-report-{i}.json').read_bytes()).hexdigest()})
(OUT/'five-rule-report-equivalence.json').write_text(json.dumps({'passed':True,'comparisons':records,'volatile_exclusions':sorted(volatile),'scope':'All static result fields and ordered findings, warnings, summaries and source coverage; Python/TypeScript/mixed existing owned fixtures, each serial and parallel. All five requested rules retain actual execution.','corpus_observations':0,'profiles':0,'paid_calls':0},indent=2)+'\n')
