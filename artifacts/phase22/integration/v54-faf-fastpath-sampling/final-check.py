...
checks=['v54-attribution','v54-owned-cleanup-settled','v54-source-assessment','v54-reconciliation','v54-final-docs','v54-final-build','v54-final-distributions']
for n in checks:assert read(BASE/(n+'.json'))['exit_code']==0,n
engineering=read(BASE/'v52-merge-fastpath/local-checks.json')['candidate_engineering_files_sha256'];assert len(engineering)==302
for n,d in engineering.items():assert sha(ROOT/n)==sha(Path('/private/tmp/mcp-phase22-frozen-a36f696')/n)==d,n
...
assert proposal['target_source_sha256']==sha(ROOT/'src/sentinel/static/path_flow.py')
...
assert not (OUT/'optimization-authorization.json').exists() and not (BASE/'v55-singleton-combine').exists()
a=read(OUT/'assessment.json');assert a['budget_closed'] and a['remaining']==0 and a['samples']==78769 and a['valid_partial_current_source_profiles']==1
...
audit=read(OUT/'audit.json');assert audit['requirements']==read(OLD/'audit.json')['requirements'] and len(audit['additional_scope_requirements'])==194
...
failed={p.stem:read(p)['exit_code'] for p in BASE.glob('v54-*.json') if 'exit_code' in read(p) and read(p)['exit_code']!=0}
assert failed=={'v54-owned-cleanup':1},failed
assert 'v54-attribution' in (BASE/'v54-owned-cleanup.log').read_text()
write('check-failure-assessment.json',{'profile_outcome':'Approved profile timed out at300seconds with four current-source worker identities and78769partial samples. No report,zero remaining.','failed_checks':failed,'explanation':'Initial read-only cleanup check correctly found the still-running evidence-attribution wrapper and helper. After those finished, the identical cleanup check passed under v54-owned-cleanup-settled. No profile retry or process termination occurred; both receipts are preserved.','corrected_receipt_sha256':sha(BASE/'v54-owned-cleanup-settled.json'),'profile_retries':0,'optimization_attempts':0,'paid_calls':0})
write('command-provenance.json',{'wrapper_commands':{n:read(BASE/(n+'.json')) for n in checks+['v54-owned-cleanup']},'direct_commands':[{'command':['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python','-I',str(OLD/'authorize-diagnostic.py')],'exit_code':0,'evidence':'v53-fastpath-regression/diagnostic-preflight.json'},{'command':['/Users/bashaarjavaid/Projects/MCP-Sentinel/.venv/bin/python','-I',str(OLD/'diagnostic.py')],'exit_code':1,'evidence':'v54-faf-fastpath-sampling/execution.json'}],'failure_assessment_sha256':sha(OUT/'check-failure-assessment.json'),'paid_calls':0})
write('final-checks.json',{'passed':True,'recorded_at':datetime.now(timezone.utc).isoformat(),'scanner':proposal['scanner'],'engineering_files_verified':302,'prior_evidence_files_verified':len(prior),'requirements':283,'protected_user_files':8,'owning_docs':16,'main_and_parent_preserved':True,'checks_sha256':{n:sha(BASE/(n+'.json')) for n in checks},'optimization_proposal_sha256':sha(OUT/'optimization-proposal.json'),'current_source_profile_attempts':1,'valid_partial_current_source_profiles':1,'current_optimization_attempts':0,'paid_calls':0,'technical_acceptance_received':False,'phase22_complete':False})
print('Final checks pass:283requirements,302unchanged engineering files,valid partial profile closed,source-only optimization unapproved.')
