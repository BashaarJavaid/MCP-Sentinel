...
def main():
    assert json.loads((BASE / "v89-regression-preparation/final-checks.json").read_text())["passed"]
    assert json.loads((BASE / "v89-regression-preparation/owned-final.json").read_text())["owned_processes"] == []
    ...
    assert not json.loads((BASE / "v89-regression-preparation/audit.json").read_text())["technical_acceptance_received"]
    ...
    archive, manifest = BASE / "evidence-v104.tar.gz", BASE / "evidence-v104.json"
    ...
    packet = {
        ...
        "files": rows,
        "previous_packet": "evidence-v103.json",
        ...
        "status": "V87completed111observations/48equal ordered pairs at884d376; all raw findings/deltas fully assessed, Proxmox still failed0/2detection and0/4negative support. Literal metadata correction engineering passes at5142a3f:2596tests/36skips locally andall12hosted suites,29normaljobs/docs,sixzero-call requests,19runtimebindings. Exact6Proxmox observations/3records/3pairs prepared but unapproved and unexecuted.105prior observations retain original source/report bindings with unchanged execution-path proof. All479requirements/28unaccepted limitations and historical failures retained. OrdinaryFAF1800completion unestablished;V68unapproved/unimplemented. Technical acceptance and accepted closeout pending;zero paid calls.",
        "verification": (
            "All 103 prior archives and every new member verified afte"
