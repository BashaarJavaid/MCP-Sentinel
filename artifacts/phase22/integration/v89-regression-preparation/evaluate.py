"""Proposed6-observation Proxmox exposed regression sequence; no execution without a new approval."""
...
def verify():
    ...
    assert proposal["bounds"]["native_observations"] == len(proposal["order"]) == 6
    assert len({(x["group"], x["input_id"]) for x in proposal["order"]}) == 3
...
def run(out):
    ...
    try:
        ...
        assert len(packet["attempts"]) == proposal["bounds"]["native_observations"] == 6 and not packet["unstarted"]
        packet["passed"] = True
