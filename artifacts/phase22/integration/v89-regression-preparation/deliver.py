...
EXPECTED='5142a3fbd5bcff63719df6cce3a30543214e7d27';TITLE='Phase 22: metadata correction verified; six-observation approval pending'
...
assert not (OUT/'execution-consumed.json').exists() and not (OUT/'evaluation-authorization.json').exists() and not (BASE/'v90-regression-results').exists()
...
seal=read(BASE/'evidence-v104.json');assert sha(BASE/'evidence-v104.json')==b['evidence_seal']['index_sha256'] and sha(BASE/seal['archive'])==seal['sha256']==b['evidence_seal']['archive_sha256']
...
assert current['headRefOid']==EXPECTED and current['title']=='Phase 22: literal metadata correction; engineering in progress'
assert current['body']==(BASE/'v88-literal-metadata-correction/candidate-pr-body.md').read_text()
...
extra=[BASE/'evidence-v104.json',BASE/'evidence-v104.tar.gz',OUT/'documentation-binding.json',OUT/'pre-delivery-pr37.json',*OUT.glob('*.py'),*BASE.glob('v89-documentation-binding.*')]
...
print(run(['git','commit','-m','Deliver failed regression assessment and six-observation correction proposal [skip ci]']),flush=True)
