...
order=[{'group':'synthetic','input_id':'fixture-'+str(n),'batch':batch} for batch in ['first','repeat'] for n in range(3)]
proposal={'order':order,'scanner':{'synthetic':True},'bounds':{'internal_stop_minutes':199,'native_observations':6}}
results=[]
...
for scenario in scenarios:
 with tempfile.TemporaryDirectory(prefix='phase22-unseen-loop-synthetic-') as tmp:
  ...
  p={**proposal,'bounds':{**proposal['bounds'],'internal_stop_minutes':1 if scenario=='insufficient_outer_allowance' else 199}}
  def supervise(command,log,limit):
   ...
   if scenario=='ordered_repeat_failure' and calls[0]==4:report['warnings'].append('changed')
  ...
  packet=json.loads((raw/'packet.json').read_text());expected=6 if scenario=='complete_with_failed_conditions' else 4 if scenario=='ordered_repeat_failure' else 0 if scenario=='insufficient_outer_allowance' else 2
  assert len(packet['attempts'])==expected and len(packet['unstarted'])==6-expected
  ...
  assert sum(x.get('ordered_repeat_equal',False) for x in packet['attempts'])==(3 if packet['passed'] else 0)
  results.append({'scenario':scenario,'attempted':expected,'closed_unstarted':6-expected,'passed':True})
