...
def main():
    ...
    packet = {"recorded_at": datetime.now(timezone.utc).isoformat(), "head": git("rev-parse", "HEAD")}
    ...
    assert {**identity, "revision": proposal["scanner"]["revision"]} == proposal["scanner"]
    ...
    assert not subprocess.check_output(["git", "status", "--porcelain", "--untracked-files=no"], cwd=frozen)
    inputs = json.loads((BASE / "v22-delivery-verification.json").read_text())["tested_inputs_identical_to_candidate"]
    assert not git("diff", "61b19ae", "HEAD", "--", *inputs)
    packet["tested_input_reuse"] = {"source": "61b19aeb56e087ebfeb52a7865aea50deb35e4bd", "byte_identical_paths": inputs}
    ...
    packet["user_files_sha256"] = {p.relative_to(ROOT).as_posix(): sha(p) for p in sorted((ROOT / "docs").glob("phase22-*prompt.md"))}
    for name, digest in json.loads((BASE / "v22-source-verification.json").read_text())["user_files_sha256"].items():
        assert sha(ROOT / name) == digest, name
    packet["environment"] = {"python": sys.version, "packages": {n: importlib.metadata.version(n) for n in ("semgrep", "mcp", "pytest", "pydantic")}}
    manifest = validate(CORPUS / "manifest.json")
    old = json.loads((ROOT / "artifacts/phase22/corpus-replacement-v3/manifest.json").read_text())
    retained = [i.model_dump(mode="json") for i in manifest.inputs if i.id not in proposal["input_order"]]
    assert retained == old["inputs"] and len(retained) == 45
    ...
    with tempfile.TemporaryDirectory(prefix="phase22-v23-source-only-") as temporary:
        for item in manifest.inputs:
            if item.id not in proposal["input_order"]:
                continue
            source = materialize(item, snapshots[item.snapshot], Path(temporary).resolve() / item.id, manifest.packet)
            config = load_configuration(source, environ={}, static_only=True, cli_overrides={"rules_only": True, "ignore_paths": [".phase20/**"], "max_findings_per_scan": 500}, llm_cli_overrides={"model": "gpt-5.6-sol", "reasoning_effort": "medium", "retries": 0, "max_concurrency": 1, "cache_enabled": False})
            payload = {"scanner": config.scanner.model_dump(mode="json"), "target": None, "static_only": config.static_only, "language": config.language.value}
            assert payload == configs[item.id]["configuration"]
            assert hashlib.sha256((json.dumps(payload, indent=2, sort_keys=True) + "\n").encode()).hexdigest() == configs[item.id]["sha256"]
            packet["materialized"].append({"id": item.id, "tree_sha256": item.tree_sha256, "files": len(snapshots[item.snapshot].files), "configuration_sha256": configs[item.id]["sha256"]})
    ...
    for version in range(1, 52):
        ...
        with tarfile.open(archive) as stream:
            ...
            for member in members:
                ...
                assert hashlib.sha256(stream.extractfile(member).read()).hexdigest() == row["sha256"]
        packet["seals"][archive.name] = {"sha256": index["sha256"], "members_verified": len(rows)}
        print("Verified seal", version, flush=True)
    audit = json.loads((BASE / "v22-closeout-audit/packet.json").read_text())
    assert len(audit["requirements"]) == 89 and len(audit["additional_scope_requirements"]) == 51
    missing = []
    ...
    packet["audit"] = {"sha256": sha(BASE / "v22-closeout-audit/packet.json"), "requirements": 89, "added_scope_rows": 51, "all_references_present": True, "dispositions": audit["dispositions"]}
    packet.update(passed=True, native_observations=0, comparator_observations=0, paid_calls=0, target_execution=False, evaluation_approved=False)
