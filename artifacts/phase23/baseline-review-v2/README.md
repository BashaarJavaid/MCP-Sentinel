# Phase 23 replacement baseline: execution checkpoint

**Prepared and unexecuted.** The user accepted the seven-input source freeze;
the [source-freeze receipt](../candidate-review-v2/source-freeze.json) binds manifest SHA-256
...
The replacement is **GHSA-3q26-f695-pp76**, bounded to
`git_init.initialBranch` in `cyanheads/git-mcp-server`. Both full upstream revisions
and five derived inputs retain the accepted source conditions and Apache-2.0
notices. The potential `promisify` analysis gap remains unmeasured.
...
`d17bd9b0313c50a90fee81caf43f0f60b1eacf2c9ca027e5c526c0d634290465`
...
.venv/bin/python -m scripts.phase23_regression run \
  --proposal artifacts/phase23/baseline-review-v2/proposal.json \
  --approval artifacts/phase23/baseline-authorization-v2.json \
  --output artifacts/phase23/baseline-v2
```
...
The existing helper now selects one of the two named case/receipt pairs. It
checks the receipt path, manifest hash and case identity before observations.
`check --manifest` supports source-only verification of the replacement. No
scanner, rule, deadline, comparison, cleanup or reporting behavior changes.
Exact previously executed helper/test bytes are archived in baseline-review-v1;
the original observations retain those identities and are not relabeled.
...
| Baseline helper | `99199da846863f12403dc306ac6b8831278732050b6fea2a5f07b24bd6d30d46` |
| Helper tests | `8e525231af7e38e9e2e2bc83ea2f0341b0371e90fe0e5783d94767b4cbcf3c8b` |
...
The current case-selection adaptation passes **23 focused checks**, full configured mypy
...
reports, real synthetic child timeout/cancellation cleanup, identity/case/freeze mismatch
closing all unused budget for either case, a complete synthetic 14-observation sequence, and output
...
**Zero replacement observations, target executions or paid calls.** The original
case retains 14 completed observations and seven equal ordered pairs. Phase 23 remains
