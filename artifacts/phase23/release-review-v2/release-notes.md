...
Native reports use schema 1.7.0; SARIF remains 2.1.0. New default rules can produce additional findings. Source support remains bounded.

The reviewed advisory shell gate passes, while 56 unrelated finding occurrences remain `needs_review`. Current FAF detection/support/repeat compatibility remains unestablished after the retained 1,800-second timeout. Proxmox remains unsupported. Recovery, optimization, paid benchmarks and pilots remain deferred. All historical failures and accepted Phase 22 limitations are preserved.

Publication time and provenance will be recorded after verification. These are prepared release notes, not evidence of publication.
